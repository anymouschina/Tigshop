        ...
        foreach ($data as $shop_id => $row) {
            ...
            foreach ($row as $shipping_tpl_id => $value) {
                ...
                foreach ($all_tpl_info as $key => $v) {
                    if ($v['is_default']) {
                        ...
                        if ($this->fetchRegion($this->getRegionIds(),
                            isset($v['region_data']['area_regions']) ? $v['region_data']['area_regions'] : [])) {
                            $tpl_info = $v;
                        }
                    }
                }
                if ($tpl_info) {
                    ...
                    if (($cart_total['product_amount'] > $tpl_info['free_price'] && $tpl_info['free_price'] > 0) || $tpl_info['is_free']) {
        ...
        try {
            ...
            foreach ($item_data as $key => $value) {
                ...
                if (!empty($value['is_gift'])) {
                    app(ProductGiftService::class)->decStock($value['promotion_data']['gift_id'], $value['quantity']);
                }
            ...
            Db::rollback();
            if (strpos($e->getMessage(), "gift_stock") !== false) {
                throw new ApiException(Util::lang('赠品库存不足'));
            } else {
