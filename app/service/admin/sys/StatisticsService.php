...
class StatisticsService extends BaseService
{
    ...
    public function log(array $data): bool
    {
        ...
        if (!empty($data['shop_id'])) {
            $shopStatisticsBase = StatisticsBase::where('date', $date)->where('shop_id', $data['shop_id'])->find();
            ...
            if (!empty($data['is_new'])) {
                StatisticsBase::where('date', $date)->where('shop_id', $data['shop_id'])->inc('click_count',
                    1)->inc('visitor_count', 1)->save();
            } else {
                StatisticsBase::where('date', $date)->where('shop_id', $data['shop_id'])->inc('click_count', 1)->save();
            }
        }
        if (!empty($data['product_id']) || !empty($data['shop_category_id'])) {
