...
class GroupService extends BaseService
{
    ...
    protected function filterQuery(array $filter): object
    {
        ...
        if (!empty($filter['group_name'])) {
            $query->where('c.group_name', 'like', '%' . $filter['group_name'] . '%');
        }
    ...
    {
        $result = $this->model->where('group_id', $id)->find();
        return $result;
    }
    ...
    public function update(int $id, array $data, bool $isAdd = false): bool|int
    {
        if ($isAdd) {
            ...
            $result = $this->model->where('group_id', $id)->save($data);
            return $result !== false;
        }
    }
