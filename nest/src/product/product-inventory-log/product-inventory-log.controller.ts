...
export class ProductInventoryLogController {
  constructor(
    private readonly productinventorylogService: ProductInventoryLogService,
  ) {}
  ...
  async list(@Query() query: QueryProductInventoryLogDto) {
    ...
    const filterResult =
      await this.productinventorylogService.getFilterList(filter);
  ...
  async create(@Body() createData: CreateProductInventoryLogDto) {
    const result =
      await this.productinventorylogService.createProductInventoryLog(
        createData,
      );
    if (result) {
  ...
  async update(
    @Param("id") id: number,
    @Body() updateData: UpdateProductInventoryLogDto,
  ) {
    const result =
      await this.productinventorylogService.updateProductInventoryLog(
        id,
        updateData,
      );
    if (result) {
  ...
  async batch(@Body() batchData: any) {
    ...
    if (batchData.type === "del") {
      await this.productinventorylogService.batchDeleteProductInventoryLog(
        batchData.ids,
      );
      return ResponseUtil.success();
    } else {
  ...
  async statistics() {
    const statistics =
      await this.productinventorylogService.getProductInventoryLogStatistics();
