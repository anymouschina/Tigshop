...
import { CreateProductGroupDto, UpdateProductGroupDto, QueryProductGroupDto } from './dto/product-group.dto';
...
export class ProductGroupController {
  ...
  async create(@Body() createData: CreateProductGroupDto) {
    const result = await this.productgroupService.createProductGroup(createData);
    if (result) {
  ...
  @Put('update/:id')
  async update(@Param('id') id: number, @Body() updateData: UpdateProductGroupDto) {
    const result = await this.productgroupService.updateProductGroup(id, updateData);
    if (result) {
  ...
  @Get('statistics')
  async statistics() {
    const statistics = await this.productgroupService.getProductGroupStatistics();
