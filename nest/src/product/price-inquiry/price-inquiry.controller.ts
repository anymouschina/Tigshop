...
export class PriceInquiryController {
  ...
  async create(@Body() createData: CreatePriceInquiryDto) {
    const result =
      await this.priceInquiryService.createPriceInquiry(createData);
    if (result) {
  ...
  async update(
    @Param("id") id: number,
    @Body() updateData: UpdatePriceInquiryDto,
  ) {
    const result = await this.priceInquiryService.updatePriceInquiry(
      id,
      updateData,
    );
    if (result) {
  ...
  async reply(@Param("id") id: number, @Body() replyData: any) {
    const result = await this.priceInquiryService.replyPriceInquiry(
      id,
      replyData,
    );
    if (result) {
  ...
  async statistics() {
    const statistics =
      await this.priceInquiryService.getPriceInquiryStatistics();
