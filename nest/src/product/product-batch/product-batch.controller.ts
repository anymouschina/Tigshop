...
import { CreateProductBatchDto, UpdateProductBatchDto, QueryProductBatchDto } from './dto/product-batch.dto';
...
export class ProductBatchController {
  ...
  async create(@Body() createData: CreateProductBatchDto) {
    const result = await this.productbatchService.createProductBatch(createData);
    if (result) {
  ...
  @Put('update/:id')
  async update(@Param('id') id: number, @Body() updateData: UpdateProductBatchDto) {
    const result = await this.productbatchService.updateProductBatch(id, updateData);
    if (result) {
  ...
  @Get('statistics')
  async statistics() {
    const statistics = await this.productbatchService.getProductBatchStatistics();
