...
export class BrandController {
  ...
  async getBrandList(@Query() query: BrandQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging
        ? this.brandService.getFilterCount(query)
  ...
  async updateBrandField(@Body() updateDto: UpdateBrandFieldDto) {
    const result = await this.brandService.updateField(
      updateDto.id,
      updateDto.field,
      updateDto.value,
    );
  ...
  async auditBrand(@Body() auditDto: AuditBrandDto) {
    const result = await this.brandService.auditBrand(
      auditDto.brand_id,
      auditDto.status,
      auditDto.reject_remark,
    );
