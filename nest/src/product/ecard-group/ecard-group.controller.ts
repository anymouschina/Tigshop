...
import { CreateECardGroupDto, UpdateECardGroupDto, QueryECardGroupDto } from './dto/ecard-group.dto';
...
export class ECardGroupController {
  ...
  @Put('update/:id')
  async update(@Param('id') id: number, @Body() updateData: UpdateECardGroupDto) {
    const result = await this.ecardgroupService.updateECardGroup(id, updateData);
    if (result) {
