...
export class ECardGroupService {
  ...
  async createECardGroup(createData: CreateECardGroupDto) {
    try {
      ...
      this.logger.debug("创建电子卡券分组失败:", error);
      return null;
    }
  }
  ...
  async updateECardGroup(id: number, updateData: UpdateECardGroupDto) {
    try {
      ...
      this.logger.debug("更新电子卡券分组失败:", error);
      return null;
    }
  }
  ...
  async deleteECardGroup(id: number) {
    try {
      ...
      this.logger.debug("删除电子卡券分组失败:", error);
      return false;
    }
  }
  ...
  async batchDeleteECardGroup(ids: number[]) {
    try {
      ...
      this.logger.debug("批量删除电子卡券分组失败:", error);
      return false;
    }
  }
  ...
  async getECardGroupStatistics() {
    try {
      ...
      this.logger.debug("获取电子卡券分组统计失败:", error);
      return {
        total: 0,
        today_count: 0,
      };
