...
export class ProductServicesController {
  ...
  async list(@Query() query: QueryProductServicesDto) {
    ...
    const filterResult =
      await this.productservicesService.getFilterList(filter);
  ...
  async create(@Body() createData: CreateProductServicesDto) {
    const result =
      await this.productservicesService.createProductServices(createData);
    if (result) {
  ...
  async update(
    @Param("id") id: number,
    @Body() updateData: UpdateProductServicesDto,
  ) {
    const result = await this.productservicesService.updateProductServices(
      id,
      updateData,
    );
    if (result) {
  ...
  async batch(@Body() batchData: any) {
    ...
    if (batchData.type === "del") {
      await this.productservicesService.batchDeleteProductServices(
        batchData.ids,
      );
      return ResponseUtil.success();
    } else {
  ...
  async statistics() {
    const statistics =
      await this.productservicesService.getProductServicesStatistics();
