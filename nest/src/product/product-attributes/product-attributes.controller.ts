...
export class ProductAttributesController {
  ...
  async list(@Query() query: QueryProductAttributesDto) {
    ...
    const filterResult =
      await this.productattributesService.getFilterList(filter);
  ...
  async create(@Body() createData: CreateProductAttributesDto) {
    const result =
      await this.productattributesService.createProductAttributes(createData);
    if (result) {
  ...
  async update(
    @Param("id") id: number,
    @Body() updateData: UpdateProductAttributesDto,
  ) {
    const result = await this.productattributesService.updateProductAttributes(
      id,
      updateData,
    );
    if (result) {
  ...
  async batch(@Body() batchData: any) {
    ...
    if (batchData.type === "del") {
      await this.productattributesService.batchDeleteProductAttributes(
        batchData.ids,
      );
      return ResponseUtil.success();
    } else {
  ...
  async statistics() {
    const statistics =
      await this.productattributesService.getProductAttributesStatistics();
