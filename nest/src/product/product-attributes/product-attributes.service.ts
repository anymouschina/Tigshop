...
export class ProductAttributesService {
  ...
  async createProductAttributes(createData: CreateProductAttributesDto) {
    try {
      ...
      this.logger.debug("创建产品属性失败:", error);
      return null;
    }
  }
  ...
  async updateProductAttributes(
    ...
    try {
      ...
      this.logger.debug("更新产品属性失败:", error);
      return null;
    }
  }
  ...
  async deleteProductAttributes(id: number) {
    try {
      ...
      this.logger.debug("删除产品属性失败:", error);
      return false;
    }
  }
  ...
  async batchDeleteProductAttributes(ids: number[]) {
    try {
      ...
      this.logger.debug("批量删除产品属性失败:", error);
      return false;
    }
  }
  ...
  async getProductAttributesStatistics() {
    try {
      ...
      this.logger.debug("获取产品属性统计失败:", error);
      return {
        total: 0,
        today_count: 0,
      };
