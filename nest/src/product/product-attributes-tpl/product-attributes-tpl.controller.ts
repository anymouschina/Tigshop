...
import { CreateProductAttributesTplDto, UpdateProductAttributesTplDto, QueryProductAttributesTplDto } from './dto/product-attributes-tpl.dto';
...
export class ProductAttributesTplController {
  constructor(private readonly productattributestplService: ProductAttributesTplService) {}
  ...
  async list(@Query() query: QueryProductAttributesTplDto) {
    ...
    const filterResult = await this.productattributestplService.getFilterList(filter);
  ...
  async create(@Body() createData: CreateProductAttributesTplDto) {
    const result = await this.productattributestplService.createProductAttributesTpl(createData);
    if (result) {
  ...
  @Put('update/:id')
  async update(@Param('id') id: number, @Body() updateData: UpdateProductAttributesTplDto) {
    const result = await this.productattributestplService.updateProductAttributesTpl(id, updateData);
    if (result) {
  ...
  async batch(@Body() batchData: any) {
    ...
    if (batchData.type === 'del') {
      await this.productattributestplService.batchDeleteProductAttributesTpl(batchData.ids);
      return ResponseUtil.success();
    } else {
  ...
  @Get('statistics')
  async statistics() {
    const statistics = await this.productattributestplService.getProductAttributesTplStatistics();
