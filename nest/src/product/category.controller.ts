...
export class CategoryController {
  ...
  async getCategoryList(@Query() query: CategoryQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging
        ? this.categoryService.getFilterCount(query)
  ...
  async getParentCategoryTree(@Query("parent_id") parentId: number = 0) {
    const records =
      await this.categoryService.getCategoryTreeByParent(parentId);
  ...
  async getParentName(@Query() query: GetParentNameDto) {
    const parentName = await this.categoryService.getParentName(
      query.parent_id,
    );
  ...
  async updateCategoryField(@Body() updateDto: UpdateCategoryFieldDto) {
    const result = await this.categoryService.updateField(
      updateDto.id,
      updateDto.field,
      updateDto.value,
    );
  ...
  async moveCategoryProducts(@Body() moveDto: MoveCategoryDto) {
    const result = await this.categoryService.moveCategoryProducts(
      moveDto.id,
      moveDto.target_category_id,
    );
