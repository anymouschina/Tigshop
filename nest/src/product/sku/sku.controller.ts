...
export class SkuController {
  ...
  async updateSkuStock(@Query('id') id: number, @Body() stockUpdateDto: SkuStockUpdateDto) {
  ...
  async updateSkuPrice(@Query('id') id: number, @Body() priceUpdateDto: SkuPriceUpdateDto) {
  ...
  async getSkuStats(@Query('productId') productId?: number) {
    return this.skuService.getSkuStats(productId ? Number(productId) : undefined);
  }
  ...
  async getSkuHistory(@Query('skuId') skuId: number, @Query('page') page = 1, @Query('size') size = 10) {
    // 简化实现，返回模拟数据
    return {
      skuId,
      history: [
