...
import {
  ...
  QueryProductPromotionDto,
} from "./dto/product-promotion.dto";
...
export class ProductPromotionController {
  ...
  async list(@Query() query: QueryProductPromotionDto) {
    ...
    const filterResult =
      await this.productPromotionService.getFilterResult(filter);
  ...
  async conflictList(@Query() query: any) {
    ...
    const filterResult =
      await this.productPromotionService.getConflictList(filter);
  ...
  async config() {
    ...
    const promotionStatus =
      await this.productPromotionService.getPromotionStatus();
  ...
  async create(@Body() createData: CreateProductPromotionDto) {
    const result =
      await this.productPromotionService.createProductPromotion(createData);
    if (!result) {
  ...
  async update(
    @Param("id") id: number,
    @Body() updateData: UpdateProductPromotionDto,
  ) {
    const result = await this.productPromotionService.updateProductPromotion(
      id,
      updateData,
    );
    if (!result) {
  ...
  @Put("update-field")
  async updateField(
    @Body() updateData: { id: number; field: string; val: any },
  ) {
    ...
    await this.productPromotionService.updateProductPromotionField(id, {
      [field]: val,
    });
    return ResponseUtil.success();
  }
  ...
  async batch(@Body() batchData: any) {
    ...
    if (batchData.type === "del") {
      await this.productPromotionService.batchDeleteProductPromotion(
        batchData.ids,
      );
      return ResponseUtil.success();
    } else {
  ...
  async statistics() {
    const statistics =
      await this.productPromotionService.getPromotionStatistics();
