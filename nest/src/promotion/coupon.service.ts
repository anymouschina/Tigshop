...
export class CouponService {
  ...
  async getFilterResult(filter: any) {
    ...
    const orderBy: any = {
      [filter.sort_field || "coupon_id"]: filter.sort_order || "desc",
    };
        ...
        const list = await Promise.all(
          ids.map(
            async (id: number) =>
              [
                id,
                await this.prisma.user_coupon.count({
                  where: { coupon_id: id },
                }),
              ] as const,
          ),
  ...
  async createCoupon(createCouponDto: CreateCouponDto) {
    ...
    const data: any = {
      ...
      use_start_date: Number(
        payload.useStartDateTs ??
          parseDateToTs(payload.useStartDate ?? payload.use_start_date),
      ),
      use_end_date: Number(
        payload.useEndDateTs ??
          parseDateToTs(payload.useEndDate ?? payload.use_end_date),
      ),
      // 领取相关
      ...
      enabled_click_get:
        (payload.enabledClickGet ?? payload.enabled_click_get) ? 1 : 0,
      // 用户等级
      limit_user_rank: (() => {
  ...
  async updateCoupon(id: number, updateCouponDto: UpdateCouponDto) {
    ...
    const data: any = {};
    if (p.couponName ?? p.coupon_name)
      data.coupon_name = p.couponName ?? p.coupon_name;
    if (p.couponDesc ?? p.coupon_desc)
      data.coupon_desc = p.couponDesc ?? p.coupon_desc;
    if (p.couponType ?? p.coupon_type)
      data.coupon_type = Number(p.couponType ?? p.coupon_type);
    if (p.couponMoney ?? p.coupon_money)
      data.coupon_money = Number(p.couponMoney ?? p.coupon_money);
    if (p.couponDiscount ?? p.coupon_discount)
      data.coupon_discount = Number(p.couponDiscount ?? p.coupon_discount);
    if (p.minOrderAmount ?? p.min_order_amount)
      data.min_order_amount = Number(p.minOrderAmount ?? p.min_order_amount);
    if (p.maxOrderAmount ?? p.max_order_amount)
      data.max_order_amount = Number(p.maxOrderAmount ?? p.max_order_amount);
    if (p.sendRange ?? p.send_range)
      data.send_range = Number(p.sendRange ?? p.send_range);
    ...
    if (p.sendType ?? p.send_type)
      data.send_type = Number(p.sendType ?? p.send_type);
    if (p.delayDay ?? p.delay_day)
      data.delay_day = Number(p.delayDay ?? p.delay_day);
    if (p.sendNum ?? p.send_num)
      data.send_num = Number(p.sendNum ?? p.send_num);
    if (p.couponUnit ?? p.coupon_unit)
      data.coupon_unit = Number(p.couponUnit ?? p.coupon_unit);
    if (p.reduceType ?? p.reduce_type)
      data.reduce_type = Number(p.reduceType ?? p.reduce_type);
    if (p.limitNum ?? p.limit_num)
      data.limit_num = Number(p.limitNum ?? p.limit_num);
    if (p.isGlobal ?? p.is_global)
      data.is_global = (p.isGlobal ?? p.is_global) ? 1 : 0;
    if (p.isNewUser ?? p.is_new_user)
      data.is_new_user = (p.isNewUser ?? p.is_new_user) ? 1 : 0;
    if (p.isShow ?? p.is_show) data.is_show = (p.isShow ?? p.is_show) ? 1 : 0;
    if (p.enabledClickGet ?? p.enabled_click_get)
      data.enabled_click_get =
        (p.enabledClickGet ?? p.enabled_click_get) ? 1 : 0;
    if (p.limitUserRank ?? p.limit_user_rank) {
      ...
      if (typeof v === "string") data.limit_user_rank = v;
        ...
        data.limit_user_rank = v
          .map((x: any) =>
            typeof x === "object" && x !== null ? (x.id ?? x.rank_id ?? x) : x,
          )
          .map((x: any) => Number(x))
          .filter((n: any) => Number.isFinite(n))
          .join(",");
