...
export class CronService {
  ...
  async dailyStatistics() {
    try {
      ...
      // 获取统计数据
      const [dashboardStats, userStats, productStats, orderStats] = await Promise.all([
      ...
      await this.logService.error('Daily statistics job failed', { error: error.message });
    }
  }
  ...
  async cacheWarmup() {
    try {
      ...
      await this.logService.error('Cache warmup job failed', { error: error.message });
    }
  }
  ...
  async checkOrderStatus() {
    try {
      ...
      await this.logService.error('Order status check job failed', { error: error.message });
    }
  }
  ...
  async cleanupExpiredData() {
    try {
      ...
      this.logger.log(`Cleanup job completed. Deleted ${deletedLogsCount} old logs`);
    } catch (error) {
      this.logger.error('Cleanup job failed:', error);
      await this.logService.error('Cleanup job failed', { error: error.message });
    }
  }
  ...
  async sendWeeklyReport() {
    try {
      ...
      // 获取周统计数据
      const weeklyStats = await this.statisticService.getDashboardStats(timeRange);
      ...
      await this.logService.error('Weekly report job failed', { error: error.message });
    }
  }
  ...
  async checkInventoryWarnings() {
    try {
      ...
      this.logger.log(`Inventory warning check completed. Found ${lowStockProducts.length} low stock products`);
    } catch (error) {
      this.logger.error('Inventory warning check job failed:', error);
      await this.logService.error('Inventory warning check job failed', { error: error.message });
    }
  }
  ...
  async healthCheck() {
    try {
      ...
      await this.logService.error('Health check job failed', { error: error.message });
    }
  }
  ...
  private async warmupPopularProductsCache(): Promise<void> {
    ...
    // 缓存热门产品
    await this.redisService.set('popular_products', popularProducts, { ttl: 3600 });
  }
  ...
  private async checkAutoConfirmOrders(): Promise<void> {
    ...
    const autoConfirmDate = new Date(Date.now() - autoConfirmDays * 24 * 60 * 60 * 1000);
