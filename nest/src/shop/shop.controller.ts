...
export class ShopController {
  ...
  async getDetail(@Query("shopId") shopIdRaw: any, @Req() req: any) {
    ...
    // PHP 中 STATUS_LIST
    const STATUS_LIST: Record<number, string> = {
      1: "开业",
      4: "暂停运营",
      10: "关店",
    };
    ...
    try {
      collectCount = await this.prisma.collect_shop.count({
        where: { shop_id: shopId },
      });
      if (userId > 0) {
        const exist = await this.prisma.collect_shop.findFirst({
    ...
    const [
      ...
      this.prisma.product.findMany({
        where: { ...whereOnSale, is_hot: 1 },
        select: baseProductSelect,
        orderBy: { product_id: "desc" },
        take: 5,
      }),
      this.prisma.product.findMany({
        where: { ...whereOnSale, is_new: 1 },
        select: baseProductSelect,
        orderBy: { product_id: "desc" },
        take: 5,
      }),
      this.prisma.product.findMany({
        where: { ...whereOnSale, is_best: 1 },
        select: baseProductSelect,
        orderBy: { product_id: "desc" },
        take: 5,
      }),
    ...
    if (shop.merchant_id && shop.merchant_id > 0) {
      ...
      if (merchant) {
        ...
        let parsed: any = null;
        try {
          parsed = merchant.merchant_data
            ? JSON.parse(merchant.merchant_data)
            : null;
        } catch {
          parsed = null;
        }
        ...
        merchantData = {
          ...
          addTime: merchant.add_time
            ? new Date(merchant.add_time * 1000)
                .toISOString()
                .replace("T", " ")
                .substring(0, 19)
            : "",
    ...
    const data = {
      ...
      status: shop.status,
      statusText,
      addTime: shop.add_time
        ? new Date((shop.add_time ?? 0) * 1000)
            .toISOString()
            .replace("T", " ")
            .substring(0, 19)
        : "",
  ...
  async getDecorate(@Query("shopId") shopIdRaw: any) {
    ...
    const cfg = await this.prisma.config.findFirst({
      where: { biz_code: "productCategoryDecorateType", is_del: 0 },
    });
    ...
    if (record) {
      ...
      if (raw && typeof raw === "object") {
        ...
        if (raw.pageModule && raw.moduleList) {
          ...
          // 尝试常见字段
          if (raw.moduleList && Array.isArray(raw.moduleList))
            moduleList = raw.moduleList;
          if (raw.pageModule && typeof raw.pageModule === "object")
            pageModule = { ...emptyPageModule, ...raw.pageModule };
        }
      }
    ...
    return {
      code: 0,
      message: "success",
      data: { pageModule, moduleList, decorateId },
    };
  }
  ...
  async collect(@Body() body: any, @Req() req: any) {
    ...
    const existing = await this.prisma.collect_shop.findFirst({
      where: { shop_id: shopId, user_id: userId },
    });
    let isCollected: boolean;
    if (action === "add") {
      if (!existing)
        await this.prisma.collect_shop.create({
          data: {
            shop_id: shopId,
            user_id: userId,
      ...
      if (existing)
        await this.prisma.collect_shop.delete({
          where: { collect_id: existing.collect_id },
        });
      isCollected = false;
    } else {
      // toggle
    ...
    const collectCount = await this.prisma.collect_shop.count({
      where: { shop_id: shopId },
    });
    return {
      code: 0,
      message: "success",
      data: { shopId, isCollected, collectCount },
    };
  }
}
