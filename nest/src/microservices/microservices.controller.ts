...
import { MicroservicesService, MicroserviceType } from './microservices.service';
...
export class MicroservicesController {
  ...
  async subscribeToRedis(@Body() data: { channel: string }) {
    await this.microservicesService.subscribeToRedis(data.channel, (message) => {
      console.log(`Received message from Redis channel ${data.channel}:`, message);
    });
  ...
  async publishEvent(@Body() data: { eventType: string; eventData: any; aggregateId: string }) {
    await this.microservicesService.publishEvent(data.eventType, data.eventData, data.aggregateId);
  ...
  async getEventsByAggregate(@Param('aggregateId') aggregateId: string) {
    const events = await this.microservicesService.getEventsByAggregate(aggregateId);
    return { events };
  }
  ...
  async sendWithLoadBalancer(@Body() data: {
  ...
  async getMetrics() {
    ...
    const connectedServices = Object.values(connectionStatus).filter(connected => connected).length;

    return {
      totalServices,
      connectedServices,
      disconnectedServices: totalServices - connectedServices,
      connectionRate: totalServices > 0 ? (connectedServices / totalServices) * 100 : 0,
