...
export class AuthorityController {
  ...
  async getAuthority(@Query("type") type: string = "0", @Request() req) {
    ...
    if (adminUser.shop_id) {
      const adminUserShop = await this.authorityService[
        "prisma"
      ].admin_user_shop.findFirst({
        where: {
          admin_id: userId,
    ...
    if (type === "0") {
      // 获取树形结构
      const authorities = await this.authorityService[
        "prisma"
      ].authority.findMany({
        where,
        orderBy: [
  ...
  private buildAuthorityTree(authorities: any[], parentId: number = 0): any[] {
    ...
    for (const authority of authorities) {
      if (authority.parent_id === parentId) {
        const children = this.buildAuthorityTree(
          authorities,
  ...
  async getAuthorityDetail(@Param("id") id: number, @Request() req) {
    const userId = req.user?.userId;
    if (!userId) {
      throw new Error("用户未登录");
    }
  ...
  async getMenuPermissions(@Query("roleId") roleId?: number, @Request() req) {
    const userId = req.user?.userId;
    if (!userId) {
      throw new Error("用户未登录");
    }
  ...
  async getActionPermissions(@Query("roleId") roleId?: number, @Request() req) {
    const userId = req.user?.userId;
    if (!userId) {
      throw new Error("用户未登录");
    }
  ...
  async deleteAuthority(@Param("id") id: number, @Request() req) {
    const userId = req.user?.userId;
    if (!userId) {
      throw new Error("用户未登录");
    }
  ...
  async updateAuthorityStatus(
    ...
    const authority = await this.authorityService.updateStatus(
      body.id,
      body.status,
    );
