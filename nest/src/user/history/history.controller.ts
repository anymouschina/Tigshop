...
export class UserHistoryController {
  ...
  async getHistoryList(
    @Request() req,
    @Query() historyListDto: HistoryListDto,
  ): Promise<HistoryListResponse> {
    return this.userHistoryService.getHistoryList(
      req.user.userId,
      historyListDto,
    );
  }
  ...
  async deleteHistory(
    ...
    return this.userHistoryService.deleteHistory(
      req.user.userId,
      deleteHistoryDto,
    );
  }
  ...
  async clearHistory(
    ...
    return this.userHistoryService.clearHistory(
      req.user.userId,
      clearHistoryDto,
    );
  }
  ...
  async getHistoryDetail(
    @Request() req,
    @Query() historyDetailDto: HistoryDetailDto,
  ): Promise<HistoryResponse> {
    return this.userHistoryService.getHistoryDetail(
      req.user.userId,
      historyDetailDto,
    );
  }
  ...
  async getHistoryStats(
    @Request() req,
    @Query() historyStatsDto: HistoryStatsDto,
  ): Promise<HistoryStatsResponse> {
    return this.userHistoryService.getHistoryStats(
      req.user.userId,
      historyStatsDto,
    );
  }
  ...
  async getRecommendedProducts(
    ...
    return this.userHistoryService.getRecommendedProducts(
      req.user.userId,
  ...
  async getHistoryCount(@Request() req): Promise<{ count: number }> {
    const userInfo = await this.userHistoryService[
      "databaseService"
    ].user.findUnique({
  ...
  async checkProductInHistory(
    ...
    const viewCount = inHistory
      ? historyProductIds.indexOf(productId) + 1
      : undefined;
