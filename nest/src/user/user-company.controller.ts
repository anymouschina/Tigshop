...
export class UserCompanyController {
  ...
  async updateUserCompany(
    ...
    const result = await this.userCompanyService.updateUserCompany(
      id,
      updateDto,
    );
    return {
  ...
  async approveUserCompany(
    ...
    const result = await this.userCompanyService.approveUserCompany(
      id,
      approveDto,
    );
    return {
  ...
  async rejectUserCompany(
    ...
    const result = await this.userCompanyService.rejectUserCompany(
      id,
      rejectDto,
    );
    return {
  ...
  @ApiResponse({ status: 200, description: "审核成功" })
  async batchApprove(
    @Body() batchData: { ids: number[]; status: number; remark?: string },
  ) {
