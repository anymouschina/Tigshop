...
export class UserCouponService {
  ...
  async getUserCouponList(userId: number, userCouponListDto: UserCouponListDto): Promise<CouponListResponse> {
    const { page = 1, size = 15, sort_field = 'start_date', sort_order = 'asc', status } = userCouponListDto;
    ...
    // 批量获取优惠券明细
    const couponIds = Array.from(new Set(userCoupons.map((uc) => uc.coupon_id))) as number[];
    const coupons = await this.databaseService.coupon.findMany({ where: { coupon_id: { in: couponIds } } });
  ...
  async getAvailableCouponList(availableCouponListDto: AvailableCouponListDto): Promise<CouponListResponse> {
    const { page = 1, size = 15, sort_field = 'add_time', sort_order = 'desc', shop_id = -1 } = availableCouponListDto;
    ...
    const [coupons, total] = await Promise.all([
      this.databaseService.coupon.findMany({ where, skip, take: size, orderBy: { [sort_field]: sort_order as any } as any }),
  ...
  async claimCoupon(userId: number, claimCouponDto: ClaimCouponDto): Promise<SuccessResponse> {
    ...
    const nowSec = Math.floor(Date.now() / 1000);
    if (nowSec < (coupon.send_start_date || 0) || nowSec > (coupon.send_end_date || 0)) {
      throw new BadRequestException('优惠券不在领取有效期内');
    }

    const currentSendCount = await this.databaseService.userCoupon.count({ where: { coupon_id } });
    if ((coupon.limit_num || 0) > 0 && currentSendCount >= (coupon.limit_num || 0)) {
      throw new BadRequestException('优惠券已领完');
    }

    const existing = await this.databaseService.userCoupon.findFirst({ where: { user_id: userId, coupon_id } });
  ...
  async deleteUserCoupon(userId: number, deleteCouponDto: DeleteCouponDto): Promise<SuccessResponse> {
    ...
    const userCoupon = await this.databaseService.userCoupon.findFirst({ where: { id, user_id: userId } });
  ...
  async getCouponDetail(userId: number, couponDetailDto: CouponDetailDto): Promise<CouponResponse> {
    ...
    const coupon = await this.databaseService.coupon.findFirst({ where: { coupon_id: id } });
    if (!coupon) throw new NotFoundException('优惠券不存在');

    const userCoupon = await this.databaseService.userCoupon.findFirst({ where: { user_id: userId, coupon_id: id } });
    ...
    const couponDetail = {
      ...
      end_date: coupon.use_end_date,
      limit_num: coupon.limit_num,
      already_send_num: await this.databaseService.userCoupon.count({ where: { coupon_id: id } }),
  ...
  async validateCoupon(userId: number, validateCouponDto: ValidateCouponDto): Promise<CouponValidationResponse> {
    ...
    const coupon = await this.databaseService.coupon.findFirst({ where: { coupon_name: code } });
    if (!coupon) return { isValid: false, discountAmount: 0, message: '优惠券不存在' };
    ...
    const nowSec = Math.floor(Date.now() / 1000);
    if (nowSec < (coupon.use_start_date || 0) || nowSec > (coupon.use_end_date || 0)) {
      return { isValid: false, discountAmount: 0, message: '优惠券不在有效期内' };
    }

    const userCoupon = await this.databaseService.userCoupon.findFirst({
      where: { user_id: userId, coupon_id: coupon.coupon_id, used_time: 0 as any },
    });
    if (!userCoupon) return { isValid: false, discountAmount: 0, message: '您没有该优惠券或已使用' };

    if (orderAmount < Number(coupon.min_order_amount)) {
      return {
        isValid: false,
        discountAmount: 0,
        message: `订单金额未达到最低消费金额 ${coupon.min_order_amount} 元` as any,
      };
    }
    ...
    if (coupon.coupon_type === 1) {
      ...
      if (coupon.max_order_amount && discountAmount > Number(coupon.max_order_amount)) {
  ...
  async useCoupon(userId: number, useCouponDto: UseCouponDto): Promise<SuccessResponse> {
    ...
    return { message: '优惠券使用成功', discountAmount: validation.discountAmount as any } as any;
  }

  // 获取用户优惠券数量统计
  async getCouponCount(userId: number): Promise<{ available: number; used: number; expired: number; total: number }> {
    const nowSec = Math.floor(Date.now() / 1000);
    const [available, used, expired] = await Promise.all([
      this.databaseService.userCoupon.count({ where: { user_id: userId, used_time: 0 as any, end_date: { gte: nowSec } as any } }),
      this.databaseService.userCoupon.count({ where: { user_id: userId, used_time: { gt: 0 } as any } }),
      this.databaseService.userCoupon.count({ where: { user_id: userId, used_time: 0 as any, end_date: { lt: nowSec } as any } }),
    ]);
  ...
  private getCouponStatus(userCoupon: any): string {
    ...
    if (Math.floor(Date.now() / 1000) > (userCoupon.end_date || 0)) return CouponStatus.EXPIRED;
    return CouponStatus.AVAILABLE;
  }

  // 内部：检查是否可领取
  private async checkCanReceive(userId: number, couponId: number): Promise<boolean> {
    const coupon = await this.databaseService.coupon.findFirst({ where: { coupon_id: couponId } });
    if (!coupon || coupon.is_delete !== false || coupon.is_show !== 1) return false;

    const nowSec = Math.floor(Date.now() / 1000);
    if (nowSec < (coupon.send_start_date || 0) || nowSec > (coupon.send_end_date || 0)) return false;

    const currentSendCount = await this.databaseService.userCoupon.count({ where: { coupon_id: couponId } });
    if ((coupon.limit_num || 0) > 0 && currentSendCount >= (coupon.limit_num || 0)) return false;

    const existing = await this.databaseService.userCoupon.findFirst({ where: { user_id: userId, coupon_id: couponId } });
    return !existing;
  }
}
