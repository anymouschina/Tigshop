...
import {
  CreateUserPointsLogDto,
  BatchDeleteDto,
} from "./dto/user-points-log.dto";
...
export class UserPointsLogController {
  ...
  async list(@Query() query: any) {
    ...
    const filterResult =
      await this.userPointsLogService.getFilterResult(filter);
  ...
  async create(@Body() createData: CreateUserPointsLogDto) {
    const result =
      await this.userPointsLogService.createUserPointsLog(createData);
    if (!result) {
