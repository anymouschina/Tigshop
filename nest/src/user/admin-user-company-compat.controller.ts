...
export class AdminUserCompanyCompatController {
  ...
  async list(@Query() q: any) {
    ...
    const where: any = {};
    if (!Number.isNaN(type) && type !== undefined && type !== 0) where.type = type;
    if (!Number.isNaN(status) && status !== undefined && status !== 0) where.status = status;
    // 无联表：用户名通过后续 enrichment，可选

    const skip = (page - 1) * size;
    const [rows, total] = await Promise.all([
      (this.prisma as any).user_company.findMany({ where, skip, take: size, orderBy: { [sortField]: sortOrder } }),
    ...
    if (username) {
      ...
      const users = userIds.length
        ? await (this.prisma as any).user.findMany({ where: { user_id: { in: userIds } }, select: { user_id: true, username: true } })
        : [];
    ...
    return { code: 0, message: "success", data: { records, total: username ? records.length : total, size, current: page, pages: Math.ceil((username ? records.length : total) / size) } };
  }
  ...
  async detail(@Query("id") idStr?: string) {
    ...
    const item = await (this.prisma as any).user_company.findUnique({ where: { id } });
  ...
  async audit(@Body() dto: any) {
    ...
    const data: any = { status: Number(dto.status ?? 0), audit_remark: String(dto.audit_remark ?? ""), audit_time: new Date() };
