...
describe("UserService", () => {
  ...
  describe("wxLogin", () => {
    ...
    it("should throw error when WeChat config is missing", async () => {
      jest
        .spyOn(configService, "wechatAppId", "get")
        .mockReturnValue("default_app_id");
  ...
  describe("referralUser", () => {
    ...
    it("should throw error when user not found", async () => {
      ...
      await expect(
        service.referralUser(userId, mockReferralDto),
    ...
    it("should throw error when user has no openId", async () => {
      ...
      await expect(
        service.referralUser(userId, mockReferralDto),
      ).rejects.toThrow(new BadRequestException("当前用户没有关联微信账号"));
    });
    ...
    it("should return error when user already has referral", async () => {
      ...
      mockDatabaseService.user.findUnique.mockResolvedValue(mockUser);
      mockDatabaseService.userReferral.findUnique.mockResolvedValue(
        existingReferral,
      );
    ...
    it("should throw error when user tries to refer themselves", async () => {
      ...
      await expect(
        service.referralUser(userId, mockReferralDto),
      ).rejects.toThrow(new BadRequestException("不能使用自己的引荐码"));
    });
  });
  ...
  describe("createReferralCode", () => {
    ...
    it("should return error when refCode already exists", async () => {
      ...
      mockDatabaseService.referralCode.findUnique.mockResolvedValue(
        existingCode,
      );

      const result = await service.createReferralCode(
        mockCreateReferralCodeDto,
      );
    ...
    it("should create referral code successfully", async () => {
      ...
      mockDatabaseService.referralCode.create.mockResolvedValue(
        newReferralCode,
      );
  ...
  describe("registerByEmail", () => {
    it("should throw error when email already registered", async () => {
      ...
      await expect(
        service.registerByEmail(email, password, name),
  ...
  describe("loginByUsernameOrEmail", () => {
    it("should login with email", async () => {
      ...
      const result = await service.loginByUsernameOrEmail(
        usernameOrEmail,
        password,
      );
