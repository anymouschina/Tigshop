...
describe("UserController", () => {
  ...
  beforeEach(async () => {
    ...
    adminReferralController = module.get<AdminReferralController>(
      AdminReferralController,
    );
    ...
    emailVerificationService = module.get(
  ...
  describe("UserController", () => {
    describe("getOrders", () => {
      it("should throw BadRequestException when service returns error", async () => {
        const userId = 1;
        mockUserService.getOrders.mockResolvedValue({
          error: { message: "User not found" },
        });

        await expect(userController.getOrders(userId)).rejects.toThrow(
          BadRequestException,
        );
      });
    ...
    describe("logout", () => {
      ...
      it("should throw UnauthorizedException when authorization header is missing", async () => {
        await expect(userController.logout(mockUser, "")).rejects.toThrow(
          UnauthorizedException,
        );
      });
      ...
      it("should throw UnauthorizedException when authorization header format is invalid", async () => {
        await expect(
          userController.logout(mockUser, "InvalidToken"),
      ...
      it("should throw BadRequestException when token blacklisting fails", async () => {
        ...
        await expect(
          userController.logout(mockUser, authorization),
  ...
  describe("WxUserController", () => {
    ...
    describe("getUserInfo", () => {
      ...
      it("should return user info", async () => {
        ...
        expect(mockUserService.getUserInfo).toHaveBeenCalledWith(
          mockUser.userId,
        );
      });
    });
    ...
    describe("referralUser", () => {
      ...
      it("should call userService.referralUser with user ID and referral data", async () => {
        ...
        const result = await wxUserController.referralUser(
          mockUser,
          referralDto,
        );

        expect(result).toEqual(mockResult);
        expect(mockUserService.referralUser).toHaveBeenCalledWith(
          mockUser.userId,
          referralDto,
        );
      });
    ...
    describe("getReferralStats", () => {
      ...
      it("should get referral stats for current user when onlySelf is true", async () => {
        ...
        const result = await wxUserController.getReferralStats(
          mockUser,
          "true",
        );

        expect(result).toEqual(mockResult);
        expect(mockUserService.getReferralStats).toHaveBeenCalledWith(
          mockUser.userId,
        );
      });
  ...
  describe("AdminReferralController", () => {
    describe("createReferralCode", () => {
      it("should call userService.createReferralCode with provided data", async () => {
        ...
        const result = await adminReferralController.createReferralCode(
          createReferralCodeDto,
        );

        expect(result).toEqual(mockResult);
        expect(mockUserService.createReferralCode).toHaveBeenCalledWith(
          createReferralCodeDto,
        );
      });
    });
    ...
    describe("getAllReferralCodes", () => {
      it("should get active referral codes when activeOnly is true", async () => {
        ...
        const result =
          await adminReferralController.getAllReferralCodes("true");
      ...
      it("should get all referral codes when activeOnly is false", async () => {
        ...
        const result =
          await adminReferralController.getAllReferralCodes("false");
    ...
    describe("updateReferralCodeStatus", () => {
      it("should call userService.updateReferralCodeStatus with id and isActive", async () => {
        ...
        const result = await adminReferralController.updateReferralCodeStatus(
          id,
          isActive,
        );

        expect(result).toEqual(mockResult);
        expect(mockUserService.updateReferralCodeStatus).toHaveBeenCalledWith(
          id,
          isActive,
        );
      });
  ...
  describe("EmailAuthController", () => {
    describe("sendVerificationCode", () => {
      it("should throw BadRequestException when code was sent recently", async () => {
        const sendEmailCodeDto: SendEmailCodeDto = {
          email: "test@example.com",
        };
        ...
        await expect(
          emailAuthController.sendVerificationCode(sendEmailCodeDto),
        ).rejects.toThrow(new BadRequestException("请60秒后再试"));
      });
      ...
      it("should throw BadRequestException when email is already registered", async () => {
        ...
        await expect(
          emailAuthController.sendVerificationCode(sendEmailCodeDto),
        ).rejects.toThrow(new BadRequestException("该邮箱已被注册"));
      });
      ...
      it("should send verification code successfully", async () => {
        ...
        emailVerificationService.sendVerificationCode.mockResolvedValue(
          undefined,
        );

        const result =
          await emailAuthController.sendVerificationCode(sendEmailCodeDto);
    ...
    describe("registerByEmail", () => {
      it("should throw BadRequestException when verification code is invalid", async () => {
        ...
        await expect(
          emailAuthController.registerByEmail(emailRegisterDto),
        ).rejects.toThrow(new BadRequestException("验证码无效"));
      });
      ...
      it("should register user successfully", async () => {
        ...
        const result =
          await emailAuthController.registerByEmail(emailRegisterDto);
    ...
    describe("login", () => {
      it("should call userService.loginByUsernameOrEmail with provided credentials", async () => {
        ...
        const mockResult = {
          success: true,
          data: { userId: 1, token: "test_token" },
        };
