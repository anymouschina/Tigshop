...
import {
  CreateUserMessageLogDto,
  BatchDeleteDto,
} from "./dto/user-message-log.dto";
...
export class UserMessageLogController {
  ...
  async list(@Query() query: any) {
    ...
    const filterResult =
      await this.userMessageLogService.getFilterResult(filter);
  ...
  async create(@Body() createData: CreateUserMessageLogDto) {
    const result =
      await this.userMessageLogService.createUserMessageLog(createData);
    if (!result) {
  ...
  async update(@Param("id") id: number, @Body() updateData: any) {
    const result = await this.userMessageLogService.updateUserMessageLog(
      id,
      updateData,
    );
    if (!result) {
