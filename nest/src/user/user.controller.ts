...
export class UserController {
  ...
  async modifyPassword(@Request() req, @Body() passwordData: { oldPassword: string; newPassword: string }) {
    return this.userService.modifyPassword(req.user.userId, passwordData.oldPassword, passwordData.newPassword);
  }
  ...
  async modifyMobile(@Request() req, @Body() mobileData: { mobile: string; code: string }) {
    return this.userService.modifyMobile(req.user.userId, mobileData.mobile, mobileData.code);
  }
  ...
  async modifyEmail(@Request() req, @Body() emailData: { email: string; code: string }) {
    return this.userService.modifyEmail(req.user.userId, emailData.email, emailData.code);
  }
  ...
  async getHistoryProduct(@Request() req, @Query('page') page: number = 1, @Query('limit') limit: number = 10) {
  ...
  async deleteHistoryProduct(@Request() req, @Body() body: { productIds: number[] }) {
    return this.userService.deleteHistoryProduct(req.user.userId, body.productIds);
  }
  ...
  async sendPasswordChangeCode(@Request() req, @Body() body: { mobile: string }) {
    return this.userService.sendPasswordChangeCode(req.user.userId, body.mobile);
  }
