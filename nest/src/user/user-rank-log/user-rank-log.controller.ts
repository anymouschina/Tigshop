...
import { CreateUserRankLogDto, UpdateUserRankLogDto, QueryUserRankLogDto } from './dto/user-rank-log.dto';
...
export class UserRankLogController {
  ...
  @Put('update/:id')
  async update(@Param('id') id: number, @Body() updateData: UpdateUserRankLogDto) {
    const result = await this.userranklogService.updateUserRankLog(id, updateData);
    if (result) {
