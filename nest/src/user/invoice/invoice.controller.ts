...
export class InvoiceController {
  ...
  async index(@Request() req, @Query() query: { page?: number; size?: number }) {
  ...
  async getOrderList(@Request() req, @Query() query: { page?: number; size?: number }) {
    const userId = req.user.userId;
    const page = query.page || 1;
    const size = query.size || 10;
  ...
  async mailInvoice(@Request() req, @Body() body: { id: number; address: string }) {
    const userId = req.user.userId;
