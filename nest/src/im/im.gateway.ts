  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  afterInit(): void {
    ...
    setInterval(() => {
      try {
        const size =
          (this.server?.clients && (this.server.clients as any).size) || 0;
  ...
  handleConnection(client: WebSocket, req: any): void {
    try {
      ...
      this.logger.debug(
        `WS connection incoming path=${url.pathname} token=${token ? "yes" : "no"} platform=${platform || ""}`,
      );
      if (token) {
  ...
  handleDisconnect(client: WebSocket): void {
    ...
    if (ctx) {
      this.logger.log(
        `Client disconnected role=${ctx.role} userId=${ctx.userId} adminId=${ctx.adminId}`,
      );
    }
  }
  ...
  private authenticate(client: WebSocket, token: string, platform?: string) {
    try {
      const secret =
        process.env.JWT_SECRET_ADMIN || process.env.JWT_SECRET || "tigshop";
      ...
      const adminId = isAdmin
        ? Number(decoded?.userId || decoded?.admin_id || userId)
        : 0;
      const role: ClientRole = isAdmin
        ? "admin"
        : userId > 0
          ? "user"
          : "guest";
      ...
      this.send(client, EVENT_AUTH_OK, {
        role,
        userId: ctx.userId,
        adminId: ctx.adminId,
      });
      this.logger.log(
        `Client authenticated role=${role} userId=${ctx.userId} adminId=${ctx.adminId} totalClients=${(this.server?.clients && (this.server.clients as any).size) || 0}`,
      );
    } catch (e) {
  ...
  async onSend(
    ...
    try {
      const { conversationId, content, shopId, userFrom, orderId } =
        payload || {};
      ...
      this.send(client, "send_error", {
        message: (e as any)?.message || "send failed",
      });
    }
  }
  ...
  onHeartBeat(@ConnectedSocket() client: WebSocket) {
    try {
      client.send(
        JSON.stringify({ type: "heartBeat", data: { ts: Date.now() } }),
      );
    } catch {}
  }
  ...
  public pushMessage(message: any) {
    try {
      ...
      if (
        (!this.server || !(this.server as any).clients) &&
        ImGateway.instance &&
        ImGateway.instance !== this
      ) {
      ...
      this.logger.log(
        `pushMessage broadcast event=message id=${message?.id} senderType=${senderType} targetUserId=${targetUserId} delivered=${delivered} clients=${(srv.clients && (srv.clients as any).size) || 0}`,
      );
    } catch (e) {
  ...
  public pushDineEvent(ev: any) {
    try {
      ...
      this.logger.log(
        `pushDineEvent kind=${ev?.kind} orderId=${ev?.orderId} delivered=${delivered}`,
      );
    } catch (e) {
