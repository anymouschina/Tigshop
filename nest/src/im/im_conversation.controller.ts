...
export class ImConversationController {
  ...
  constructor(
    private service: ImConversationService,
  ...
  async sendMessage(@Body() body: any, @Req() req: any) {
    ...
    if (role === "servant" && !servantId) {
      const currentAdminId =
        tokenUser.userId || tokenUser.admin_id || tokenUser.adminId;
    ...
    try {
      ...
      this.logger.error(
        `push ws message failed id=${data?.id}: ${(e as any)?.message}`,
      );
    }
    return { code: 0, message: "success", data };
  }
  ...
  async setRead(@Body() body: any, @Req() req: any) {
    ...
    const messageIds = Array.isArray(body.messageIds)
      ? body.messageIds.map((v: any) => Number(v))
      : undefined;
    ...
    // 推送 read 事件
    try {
      const gw: ImGateway | undefined = (ImGateway.instance ||
        (this as any).gateway) as ImGateway | undefined;
      const now = new Date();
      ...
      if (gw && typeof gw.pushReadEvent === "function") {
        gw.pushReadEvent({
          ...
          shopId: body.shopId ? Number(body.shopId) : 0,
          userId:
            (req?.user?.role === "admin" ? body.userId : req?.user?.userId) ||
            undefined,
          servantId:
  ...
  async listConversations(
    ...
    const inferredRole: "servant" | "user" =
      role === "servant"
        ? "servant"
        : role === "user"
          ? "user"
    ...
    const onlyMine =
      resolvedRole === "servant" && (mine === "1" || mine === "true");
    const currentAdminId = onlyMine
      ? (req?.user?.userId ?? req?.user?.adminId ?? req?.user?.admin_id)
      : undefined;
    // 兼容要求：status=1 代表“进行中”(内部=0)；status=2 代表“已关闭”(内部=1)。其它保持原语义/不筛选。
    let internalStatus: number | undefined = undefined;
    const statusMappingMode = false;
    ...
    const data = await this.service.listConversations({
      ...
      currentServantId:
        onlyMine && currentAdminId ? Number(currentAdminId) : undefined,
      onlyMine,
      includeMessages,
      statusMappingMode,
    });
  ...
  async transfer(@Body() body: any, @Req() req: any) {
    ...
    const originalProvided =
      body.toServantId !== undefined ||
      body.servantId !== undefined ||
      body.adminId !== undefined ||
      body.toAdminId !== undefined ||
      body.kefuId !== undefined ||
      body.toKefuId !== undefined ||
      body.targetServantId !== undefined ||
      body.targetId !== undefined ||
      body.to_servant_id !== undefined ||
      body.servant_id !== undefined ||
      body.admin_id !== undefined ||
      body.to_admin_id !== undefined ||
      body.kefu_id !== undefined ||
      body.to_kefu_id !== undefined;
    let toServantId = num(
      body.toServantId ??
        body.servantId ??
        body.adminId ??
    ...
    const fromServantId = num(
      body.fromServantId ??
        ...
        body.fromServant ??
        body.from_admin ??
        body.from_servant_id,
    );
    const force = body.force
      ? Boolean(Number(body.force) || body.force === true)
      : false;
  ...
  async accept(@Body() body: any, @Req() req: any) {
    ...
    const data = await this.service.transfer({
      conversationId,
      toServantId: Number(currentAdminId),
      acceptMode: true,
    });
  ...
  async closeConversation(@Body() body: any) {
    const data = await this.service.closeConversation({
      conversationId: Number(body.conversationId),
    });
    return { code: 0, message: "success", data };
  }
