...
export class UserRechargeOrderController {
  ...
  @ApiQuery({ name: 'status', required: false, description: '状态: 0-待支付,1-成功,2-失败' })
  ...
  async createRechargeOrder(@Request() req, @Body() body: CreateRechargeOrderDto) {
    const userId = req.user.userId;
  ...
  async getRechargeOrder(@Request() req, @Query() query: { order_id: number }) {
    const userId = req.user.userId;
    return this.userRechargeOrderService.getRechargeOrder(query.order_id, userId);
  }
  ...
  @ApiQuery({ name: 'order_id', required: true, description: '订单ID' })
  @ApiResponse({ status: 200, description: '获取成功' })
  async checkRechargeStatus(@Request() req, @Query() query: CheckRechargeStatusDto) {
    const userId = req.user.userId;
    return this.userRechargeOrderService.checkRechargeStatus(query.order_id, userId);
  }
  ...
  async handlePaymentCallback(@Query() query: { pay_log_id: number }, @Body() paymentData: any) {
    return this.userRechargeOrderService.handlePaymentCallback(query.pay_log_id, paymentData);
  }
}