...
export class UserAccountController {
  ...
  async withdrawApply(@Request() req, @Body() withdrawDto: WithdrawApplyDto) {
    const userId = req.user.userId;
    const data = await this.userAccountService.withdrawApply(
      userId,
      withdrawDto,
    );
    return {
  ...
  async rechargeOrder(@Request() req, @Body() rechargeDto: RechargeOrderDto) {
    const userId = req.user.userId;
    const data = await this.userAccountService.rechargeOrder(
      userId,
      rechargeDto,
    );
    return {
  ...
  async setWithdrawPassword(
    @Request() req,
    @Body() setPasswordDto: SetWithdrawPasswordDto,
  ) {
    const userId = req.user.userId;
    const data = await this.userAccountService.setWithdrawPassword(
      userId,
      setPasswordDto,
    );
    return {
  ...
  async verifyWithdrawPassword(
    @Request() req,
    @Body() verifyDto: VerifyWithdrawPasswordDto,
  ) {
    const userId = req.user.userId;
    const data = await this.userAccountService.verifyWithdrawPassword(
      userId,
      verifyDto,
    );
    return {
  ...
  async getWithdrawList(@Request() req, @Query() queryDto: BalanceQueryDto) {
    const userId = req.user.userId;
    const data = await this.userAccountService.getWithdrawList(
      userId,
      queryDto,
    );
    return {
  ...
  async getRechargeList(@Request() req, @Query() queryDto: BalanceQueryDto) {
    const userId = req.user.userId;
    const data = await this.userAccountService.getRechargeList(
      userId,
      queryDto,
    );
    return {
