import { Controller, Get, Post, Put, Delete, Body, Query, Request, UseGuards, Param } from '@nestjs/common';
...
export class UserCouponController {
  ...
  async getUserCouponList(@Request() req, @Query() queryDto: CouponQueryDto) {
    ...
    const data = await this.userCouponService.getUserCouponList(userId, queryDto);
    return {
      code: 200,
      message: '获取成功',
      data,
  ...
  async getAvailableCouponList(
    ...
    const userId = req.user.userId;
    const data = await this.userCouponService.getAvailableCouponList(userId, productId, categoryId);
    return {
      code: 200,
      message: '获取成功',
      data,
  ...
  @ApiResponse({ status: 200, description: '获取成功' })
  async getCouponDetail(@Request() req, @Param('userCouponId') userCouponId: number) {
    const userId = req.user.userId;
    const data = await this.userCouponService.getCouponDetail(userId, userCouponId);
    return {
      code: 200,
      message: '获取成功',
      data,
  ...
  async useCoupon(
    ...
    const userId = req.user.userId;
    const data = await this.userCouponService.useCoupon(userId, userCouponId, orderId);
    return {
      code: 200,
      message: '使用成功',
      data,
  ...
  async calculateCouponDiscount(
    ...
    const userId = req.user.userId;
    const data = await this.userCouponService.calculateCouponDiscount(userId, couponId, orderAmount);
    return {
      code: 200,
      message: '计算成功',
      data,
