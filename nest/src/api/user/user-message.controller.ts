...
export class UserMessageController {
  ...
  @ApiResponse({ status: 200, description: '获取成功' })
  async getMessageDetail(@Request() req, @Param('messageId') messageId: number) {
    const userId = req.user.userId;
    const data = await this.userMessageService.getMessageDetail(userId, messageId);
    return {
      code: 200,
      message: '获取成功',
      data,
  ...
  async batchDeleteMessages(@Request() req, @Body() batchDto: MessageBatchDto) {
    const userId = req.user.userId;
    const data = await this.userMessageService.batchDeleteMessages(userId, batchDto);
    return {
      code: 200,
      message: '批量删除成功',
      data,
  ...
  async batchMarkAsRead(@Request() req, @Body() batchDto: MessageBatchDto) {
    const userId = req.user.userId;
    const data = await this.userMessageService.batchMarkAsRead(userId, batchDto);
    return {
      code: 200,
      message: '批量标记成功',
      data,
