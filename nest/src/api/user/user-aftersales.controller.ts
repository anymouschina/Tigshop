...
export class UserAftersalesController {
  ...
  async getAfterSalesOrderList(@Request() req, @Query() queryDto: AftersalesQueryDto) {
    const userId = req.user.userId;
    return await this.userAftersalesService.getAfterSalesOrderList(userId, queryDto);
  }
  ...
  async getAfterSalesApplyData(@Query() applyDataDto: ApplyDataDto) {
    return await this.userAftersalesService.getAfterSalesApplyData(applyDataDto);
  }

  @Post('create')
  @ApiOperation({ summary: '创建售后申请' })
  @ApiResponse({ status: 200, description: '申请成功' })
  async createAfterSales(@Request() req, @Body() createDto: CreateAftersalesDto) {
    const userId = req.user.userId;
  ...
  async updateAfterSales(@Request() req, @Body() updateDto: UpdateAftersalesDto) {
    const userId = req.user.userId;
  ...
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'size', required: false, type: Number })
  async getAfterSalesRecord(@Request() req, @Query() queryDto: AftersalesQueryDto) {
    const userId = req.user.userId;
    return await this.userAftersalesService.getAfterSalesRecord(userId, queryDto);
  }
  ...
  async getAfterSalesDetail(@Request() req, @Param('id') id: string) {
    ...
    return await this.userAftersalesService.getAfterSalesDetail(aftersaleId, userId);
  }
  ...
  async submitFeedback(@Request() req, @Body() feedbackDto: AftersalesFeedbackDto) {
    const userId = req.user.userId;
  ...
  async cancelAfterSales(@Request() req, @Query('aftersale_id') aftersale_id: string) {
    const userId = req.user.userId;
    const aftersalesId = parseInt(aftersale_id);
    return await this.userAftersalesService.cancelAfterSales(userId, aftersalesId);
  }
}