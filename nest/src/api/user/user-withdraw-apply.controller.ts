...
export class UserWithdrawApplyController {
  constructor(private readonly userWithdrawApplyService: UserWithdrawApplyService) {}

  @Get()
  @ApiOperation({ summary: '获取提现申请列表' })
  @ApiQuery({ name: 'page', required: false, description: '页码' })
  @ApiQuery({ name: 'size', required: false, description: '每页数量' })
  @ApiQuery({ name: 'status', required: false, description: '状态: 0-待审核,1-已通过,2-已拒绝' })
  ...
  @ApiQuery({ name: 'account_type', required: false, description: '账号类型:1-银行卡,2-支付宝,3-微信,4-PayPal' })
  @ApiQuery({ name: 'account_id', required: false, description: '账号ID' })
  @ApiResponse({ status: 200, description: '获取成功' })
  async getWithdrawAccountList(@Request() req, @Query() query: WithdrawAccountQueryDto) {
    const userId = req.user.userId;
  ...
  async createWithdrawAccount(@Request() req, @Body() body: CreateWithdrawAccountDto) {
    const userId = req.user.userId;
  ...
  async updateWithdrawAccount(
    ...
    const userId = req.user.userId;
    return this.userWithdrawApplyService.updateWithdrawAccount(userId, accountId, body);
  }

  @Get('accounts/:id')
  @ApiOperation({ summary: '获取提现账号详情' })
  @ApiResponse({ status: 200, description: '获取成功' })
  async getWithdrawAccountDetail(@Request() req, @Param('id') accountId: number) {
    const userId = req.user.userId;
    return this.userWithdrawApplyService.getWithdrawAccountDetail(userId, accountId);
  }
  ...
  async deleteWithdrawAccount(@Request() req, @Param('id') accountId: number) {
    const userId = req.user.userId;
    return this.userWithdrawApplyService.deleteWithdrawAccount(userId, accountId);
  }

  @Post('apply')
  @ApiOperation({ summary: '创建提现申请' })
  @ApiResponse({ status: 200, description: '申请成功' })
  async createWithdrawApply(@Request() req, @Body() body: CreateWithdrawApplyDto) {
    const userId = req.user.userId;
