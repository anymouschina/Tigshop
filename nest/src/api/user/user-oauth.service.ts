...
export class UserOauthService {
  ...
  private async getQQUserInfo(
    ...
    try {
      ...
      // 获取openid
      const openidResponse = await this.httpService.axiosRef.get(
      ...
      const openidData = JSON.parse(
        openidResponse.data.replace(/^[^{]*({.*})[^}]*$/, "$1"),
      );
      if (openidData.error) {
        throw new BadRequestException(
          `获取QQ openid失败: ${openidData.error_description}`,
        );
      }
  ...
  private async getWeiboUserInfo(
    ...
    try {
      ...
      if (tokenData.error) {
        throw new BadRequestException(
          `微博授权失败: ${tokenData.error_description}`,
        );
      }
