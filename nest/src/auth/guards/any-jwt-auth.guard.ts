// @ts-nocheck
import { Injectable, ExecutionContext, HttpException, Logger } from '@nestjs/common';
...
export class AnyJwtAuthGuard extends AuthGuard(['jwt', 'admin-jwt']) {
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    this.logger.debug(`AuthGuard handleRequest path=${request.url} authHeaderPresent=${!!authHeader} strategies=[jwt,admin-jwt] err=${err?err.message||err.name:'none'} info=${info? (info.message||info.name||JSON.stringify(info)):'none'} userPresent=${!!user}`);

    if (err || !user) {
      // 针对 admin 策略失败但仍可能普通用户可用的场景：如果 info 指向 admin 角色缺失，尝试从解析过的 request.user (有的 passport 会附加) 回退
      if (!user && request.user) {
        this.logger.warn(`Fallback using request.user after strategy error: keys=${Object.keys(request.user).join(',')}`);
    ...
    if (!user) {
      ...
      this.logger.warn(`Unauthorized: issuing 401 for path=${request.url} reason=${err?.message || info?.message || 'NO_USER'}`);
    ...
    this.logger.debug(`AuthGuard normalized user userId=${user.userId} role=${user.role}`);
    return user;
  }
}
