...
export class AdminJwtStrategy extends PassportStrategy(Strategy, "admin-jwt") {
  ...
  async validate(req: Request, payload: AdminJwtPayload) {
    this.logger.debug(
      `Admin JWT validate called with payload: ${JSON.stringify(payload)}`,
    );
    ...
    if (!payload.role || payload.role !== "admin") {
      this.logger.error(
        `Invalid admin token: role is missing or not admin. Role: ${payload.role}`,
      );
    ...
    this.logger.debug(
      `Looking for admin user with admin_id: ${payload.userId}, found: ${!!adminUser}`,
    );
    ...
    this.logger.debug(
      `Admin user found: ${JSON.stringify({
