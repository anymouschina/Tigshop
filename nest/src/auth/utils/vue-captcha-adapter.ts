...
export class VueCaptchaAdapter {
   ...
   * 获取验证码
   */
  static async getCaptcha(params: VueCaptchaRequest = {}): Promise<VueCaptchaResponse> {
    try {
   ...
   * 验证滑块
   */
  static async verifyCaptcha(params: VueVerifyRequest): Promise<VueVerifyResponse> {
    try {
   ...
   * 创建加密的pointJson
   */
  static createPointJson(x: number, y: number = 5.0, secretKey?: string): string {
    const point = { x, y };
   ...
   * 计算滑块距离
   */
  static calculateDistance(moveLeftDistance: number, imgWidth: number = 310): number {
    const STD_WIDTH = 310;
   ...
   * 生成验证令牌
   */
  static generateVerifyToken(token: string, pointStr: string, secretKey?: string): string {
...
export class VueCaptchaAPI {
   ...
   * 验证滑块
   */
  async verify(moveLeftDistance: number, imgWidth: number = 310): Promise<VueVerifyResponse> {
    const distance = VueCaptchaAdapter.calculateDistance(moveLeftDistance, imgWidth);
    const pointJson = VueCaptchaAdapter.createPointJson(distance, 5.0, this.secretKey);
   ...
   * 生成验证令牌（用于后端验证）
   */
  generateVerifyToken(moveLeftDistance: number, imgWidth: number = 310): string {
    const distance = VueCaptchaAdapter.calculateDistance(moveLeftDistance, imgWidth);
    const pointStr = JSON.stringify({ x: distance, y: 5.0 });

    return VueCaptchaAdapter.generateVerifyToken(this.token, pointStr, this.secretKey);
  }
}