...
export class PaymentCallbackController {
  ...
  async notifyByPath(
    ...
    console.log(
      `[PAY-NOTIFY] hit path notify payCode=${payCode} bodyKeys=${Object.keys(
        body || {},
      )
        .slice(0, 10)
        .join(",")}`,
    ...
    this.logger.warn(
      `[PaymentCallback] notifyByPath called with payCode=${payCode}`,
    );
    const res = await this.payService.handleNotify(
      payCode || "wechat",
      body,
      headers,
    );
  ...
  async refundNotify(
    ...
    console.log(
      `[PAY-REFUND-NOTIFY] hit refund payCode=${payCode} bodyKeys=${Object.keys(
        body || {},
      )
        .slice(0, 10)
        .join(",")}`,
    );
    const res = await this.payService.handleRefundNotify(
      payCode || "wechat",
      body,
    );
    return res;
