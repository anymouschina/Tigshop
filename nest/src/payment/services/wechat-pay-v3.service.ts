...
export class WechatPayV3Service {
  ...
  > {
    if (
      !forceRefresh &&
      this.cachedSettings &&
      Date.now() - this.cachedAt < this.SETTINGS_TTL_MS
    ) {
    ...
    const privateKeyPemInline = pick(cfg, [
      "privateKey",
      "privateKeyPem",
      "private_key",
    ]);
    let privateKeyPath = pick(cfg, ["privateKeyPath", "apiclientKeyPath"]);
    const publicKeyPemInline = pick(cfg, [
      "publicKey",
      "publicKeyPem",
      "public_key",
    ]);
    ...
    if (!publicKeyPem && publicKeyPath) {
      const abs = path.isAbsolute(publicKeyPath)
        ? publicKeyPath
    ...
    if (!notifyUrl) {
      const map = await this.settingConfig.getConfigsByCodes([
        "wechatServerUrl",
      ]);
    ...
    if (notifyUrl.includes("?")) {
      ...
      this.logger.warn(
        `移除 notifyUrl 中的查询参数: 原=${originalNotify} 现=${notifyUrl}`,
      );
    }
    if (notifyUrl.includes("#")) {
      notifyUrl = notifyUrl.split("#")[0];
    ...
    if (notifyUrl && !isAbsoluteHttp(notifyUrl)) {
      this.logger.warn(
        `无效的微信支付回调地址: ${notifyUrl}，将回退到默认 https://domain.beqlee.icu/order/pay/notify/wechat (请在 wechatPaySettings.notifyUrl 或 wechatServerUrl 正确配置完整 https:// 域名)`,
      );
    ...
    if (serialNo && derivedSerial && serialNo !== derivedSerial) {
      this.logger.warn(
        `配置中的 serialNo(${serialNo.slice(0, 8)}***) 与证书内序列号(${derivedSerial.slice(0, 8)}***) 不一致，将强制使用证书序列号进行所有签名。`,
      );
    }
    ...
    const snapshot = {
      ...
      derivedSerial: derivedSerial
        ? derivedSerial.slice(0, 8) + "***"
        : undefined,
      notifyUrl,
      apiBase,
      privateKeySource: privateKeyPemInline
        ? "inline"
        : privateKeyPath
          ? `file:${privateKeyPath}`
          : "default-cert/apiclient_key.pem",
      publicKeySource: publicKeyPemInline
        ? "inline"
        : publicKeyPath
          ? `file:${publicKeyPath}`
          : "default-cert/apiclient_cert.pem",
    };
    this.logger.debug(
      `[wechatPaySettings] 加载完成: ${JSON.stringify(snapshot)}`,
    );
  ...
  async unifiedOrderJsapi(params: {
    ...
    const {
      ...
      apiV3Key,
      spMchId,
      subMchId: cfgSubMchId,
      subAppId: cfgSubAppId,
    } = await this.getWechatPaySettings();
    ...
    if (params.timeExpire) {
      const expireStr =
        params.timeExpire instanceof Date
          ? params.timeExpire.toISOString().replace(/\.\d{3}Z$/, "+00:00")
          : String(params.timeExpire);
    ...
    if (!disableNotify) {
      ...
      if (!body.notify_url) {
        this.logger.error(
          "notify_url 未配置或无效：请在 wechatPaySettings.notifyUrl 填写以 https 开头的完整地址，或在 apiSettings.wechatServerUrl 填写域名以便自动推导。",
        );
        throw new HttpException(
      ...
      this.logger.warn(
        "[wechat-unified-order] 已启用 WECHAT_DISABLE_NOTIFY=true，系统将依赖主动查询补单，请确保主动查询逻辑稳定。",
      );
    }
    ...
    const debugBody = {
      ...
      payer: isServiceProvider
        ? {
            ...(body.payer.sub_openid
              ? {
                  sub_openid: (body.payer.sub_openid || "").slice(0, 6) + "***",
                }
              : {}),
          }
    ...
    this.logger.debug(
      `[wechat-unified-order-sdk] transactions_jsapi body=${JSON.stringify(debugBody)}`,
    );
    ...
    if (result?.status >= 200 && result?.status < 300) {
      ...
      if (
        ...
        data.package.startsWith("prepay_id=")
      ) {
      ...
      this.logger.error(
        `微信统一下单返回异常，无 prepay_id: ${JSON.stringify(result)}`,
      );
      throw new HttpException(
        "微信统一下单失败：未返回 prepay_id",
  ...
  async unifiedOrderNative(params: {
    ...
    if (params.sceneInfo) body.scene_info = params.sceneInfo;
    if (!disableNotify) body.notify_url = notifyUrl;
    else
      this.logger.warn("[wechat-native-order] WECHAT_DISABLE_NOTIFY=true 生效");
    const debugBody = { ...body };
    this.logger.debug(
      `[wechat-native-order-sdk] body=${JSON.stringify(debugBody)}`,
    );
    ...
    if (result?.status >= 200 && result?.status < 300) {
      ...
      this.logger.error(
        `微信 Native 下单返回异常，无 code_url: ${JSON.stringify(result)}`,
      );
      throw new HttpException(
        "微信 Native 下单失败：未返回 code_url",
        HttpStatus.BAD_GATEWAY,
      );
    }
    const errText = result?.error || JSON.stringify(result);
  ...
  private async signedRequest(
    ...
    const { mchId, serialNo, derivedSerial, privateKeyPem, apiBase } =
      await this.getWechatPaySettings();
    ...
    try {
      ...
      this.logger.error(
        `生成签名失败(${method} ${urlPath}): ${(e as Error).message}`,
      );
      throw new HttpException(
        "微信请求签名失败",
        HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    try {
      ...
      this.logger.warn(
        `微信 API 响应非 2xx (${method} ${urlPath}) status=${resp.status} body=${JSON.stringify(resp.data)}`,
      );
      throw new HttpException(
        `微信接口调用失败(${resp.status})`,
        HttpStatus.BAD_GATEWAY,
      );
    } catch (e) {
      if (e instanceof HttpException) throw e;
      this.logger.error(
        `微信 API 请求异常(${method} ${urlPath}): ${(e as Error).message}`,
      );
  ...
  async closeTransaction(outTradeNo: string) {
    if (!outTradeNo)
      throw new HttpException("关单缺少 outTradeNo", HttpStatus.BAD_REQUEST);
  ...
  async queryRefund(refundNo: string) {
    if (!refundNo)
      throw new HttpException("查询退款缺少 refundNo", HttpStatus.BAD_REQUEST);
  ...
  verifyNotifySignature(
    ...
    // TODO: 接入平台证书后完善。
    this.logger.debug(
      "[verifyNotifySignature] 暂未启用平台证书严格验签，直接返回 true",
    );
    return true;
  }
  ...
  async queryTransactionByOutTradeNo(outTradeNo: string): Promise<any | null> {
    ...
    try {
      ...
      if (isServiceProvider) {
        ...
        if (!subMchId) {
          this.logger.warn(
            `[wechat-query] 服务商模式查询缺少 sub_mchid，尝试仍按直连查询 out_trade_no=${outTradeNo}`,
          );
        }
        if (
          typeof (pay as any).partner_transactions_out_trade_no === "function"
        ) {
          ...
          this.logger.debug(
            "[wechat-query] SDK 未找到 partner_transactions_out_trade_no 方法，将回退自签请求；可用方法: " +
              Object.keys(pay)
                .filter((k) => k.includes("partner") || k.includes("trans"))
                .slice(0, 20)
                .join(","),
          );
          ...
          return await this.signedRequest(
            "GET",
            `/v3/pay/partner/transactions/out-trade-no/${outTradeNo}?sp_mchid=${spMchId || mchId}&sub_mchid=${subMchId || ""}`,
          );
        }
      } else {
        if (typeof (pay as any).transactions_out_trade_no === "function") {
          ...
          this.logger.debug(
            "[wechat-query] SDK 未找到 transactions_out_trade_no 方法，将回退自签请求；可用方法: " +
              Object.keys(pay)
                .filter((k) => k.includes("trans"))
                .slice(0, 20)
                .join(","),
          );
          return await this.signedRequest(
      ...
      this.logger.warn(
        `[wechat-query] 非 2xx 响应 out_trade_no=${outTradeNo} status=${result?.status} body=${JSON.stringify(result)}`,
      );
      return null;
    } catch (e) {
      this.logger.warn(
        `[wechat-query] 查询失败 out_trade_no=${outTradeNo}: ${(e as Error).message}`,
      );
      return null;
    }
  }
  ...
  buildJsapiPayInfo(appId: string, prepayId: string) {
    ...
    const message = `${appId}\n${timeStamp}\n${nonceStr}\n${pkg}\n`;
    return this.getWechatPaySettings().then(
      ({
        appId: cfgAppId,
        mchId,
        serialNo,
  ...
  async decryptNotifyResource(resource: {
    ...
    try {
      ...
      if (!apiV3Key || String(apiV3Key).length !== 32) {
        this.logger.error(
          "缺少有效的 apiV3Key（长度需为32字节）以解密微信通知",
        );
        return null;
      }
      ...
      const aad = resource.associated_data
        ? Buffer.from(resource.associated_data, "utf8")
        : undefined;
