...
export class AlipayService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.appId =
      this.configService.get<string>("ALIPAY_APP_ID") || "mock_app_id";
    this.privateKey =
      this.configService.get<string>("ALIPAY_PRIVATE_KEY") ||
      "mock_private_key";
    this.alipayPublicKey =
      this.configService.get<string>("ALIPAY_PUBLIC_KEY") || "mock_public_key";
    this.notifyUrl =
      this.configService.get<string>("ALIPAY_NOTIFY_URL") ||
      "https://your-domain.com/api/payments/callback";
    this.returnUrl =
      this.configService.get<string>("ALIPAY_RETURN_URL") ||
      "https://your-domain.com/payment/return";
    this.gatewayUrl =
      this.configService.get<string>("ALIPAY_GATEWAY_URL") ||
    ...
    try {
      ...
      if (response.code === "10000") {
        ...
        throw new BadRequestException(
          `支付宝支付创建失败: ${response.msg} - ${response.sub_msg}`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.code === "10000") {
        return {
          paymentSn,
          status:
            response.trade_status === "TRADE_SUCCESS" ? "PAID" : "PENDING",
        ...
        throw new BadRequestException(
          `查询支付状态失败: ${response.msg} - ${response.sub_msg}`,
        );
      }
    } catch (error) {
  ...
  async createRefund(refundData: {
    ...
    try {
      ...
      if (response.code === "10000") {
        ...
        throw new BadRequestException(
          `申请退款失败: ${response.msg} - ${response.sub_msg}`,
        );
      }
    } catch (error) {
  ...
  async queryRefund(refundSn: string) {
    try {
      ...
      if (response.code === "10000") {
        ...
        throw new BadRequestException(
          `查询退款状态失败: ${response.msg} - ${response.sub_msg}`,
        );
      }
    } catch (error) {
  ...
  async closePayment(paymentSn: string) {
    try {
      ...
      if (response.code === "10000") {
        ...
        throw new BadRequestException(
          `关闭订单失败: ${response.msg} - ${response.sub_msg}`,
        );
      }
    } catch (error) {
