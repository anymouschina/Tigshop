...
export class StatisticsFacadeService {
  ...
  async getDashboardStats(timeRange?: TimeRange) {
    const now = new Date();
    const start = timeRange?.startDate ?? new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
    const end = timeRange?.endDate ?? now;
    const ts = (d: Date) => Math.floor(d.getTime() / 1000);

    const [totalUsers, newUsersToday, totalOrders, ordersToday, totalRevenueAgg, revenueTodayAgg] = await Promise.all([
      this.prisma.user.count(),
      this.prisma.user.count({ where: { reg_time: { gte: ts(new Date(start)), lte: ts(new Date(end)) } } as any }),
      this.prisma.order.count({ where: { is_del: 0 } as any }),
      this.prisma.order.count({ where: { is_del: 0, add_time: { gte: ts(start), lte: ts(end) } } as any }),
      this.prisma.order.aggregate({ where: { is_del: 0 } as any, _sum: { paid_amount: true } }),
      this.prisma.order.aggregate({ where: { is_del: 0, add_time: { gte: ts(start), lte: ts(end) } } as any, _sum: { paid_amount: true } }),
    ]);
  ...
  async getUserStats(timeRange: TimeRange) {
    ...
    const [totalUsers, newUsers, activeUsers, userGrowthData] = await Promise.all([
      this.prisma.user.count(),
      this.prisma.user.count({ where: { reg_time: { gte: ts(timeRange.startDate), lte: ts(timeRange.endDate) } } as any }),
  ...
  async getProductStats() {
    const [totalProducts, activeProducts, lowStockProducts, outOfStockProducts] = await Promise.all([
      ...
      this.prisma.product.count({ where: { is_delete: 0, product_status: 1 } as any }),
      this.prisma.product.count({ where: { is_delete: 0, product_stock: { lt: 10 } } as any }),
      this.prisma.product.count({ where: { is_delete: 0, product_stock: 0 } as any }),
    ]);

    // 顶部畅销商品：可按需扩展，先返回空数组占位
    const topSellingProducts: Array<{ productId: number; productName: string; salesCount: number; revenue: number }> = [];
  ...
  async getOrderStats(timeRange: TimeRange) {
    const ts = (d: Date) => Math.floor(d.getTime() / 1000);
    const totalOrders = await this.prisma.order.count({ where: { is_del: 0, add_time: { gte: ts(timeRange.startDate), lte: ts(timeRange.endDate) } } as any });
    ...
    const revenueAgg = await this.prisma.order.aggregate({ where: { is_del: 0, add_time: { gte: ts(timeRange.startDate), lte: ts(timeRange.endDate) } } as any, _sum: { paid_amount: true } });
    const totalRevenue = Number(revenueAgg._sum.paid_amount || 0);
    const avgOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0;
    return { totalOrders, orderStatusDistribution, totalRevenue, avgOrderValue };
  }
