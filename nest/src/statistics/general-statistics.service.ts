...
export class GeneralStatisticsService {
    ...
    const [orders, revenue, users] = await Promise.all([
      this.prisma.order.count({
        where: {
          shop_id: shopId,
          is_del: 0,
          add_time: { gte: startTs, lte: endTs },
        } as any,
      }),
      ...
      this.prisma.user.count({
        where: { reg_time: { gte: startTs, lte: endTs } } as any,
      }),
    ]);

    const totalProducts = await this.prisma.product.count({
      where: { shop_id: shopId, is_delete: 0 } as any,
    });
  ...
  async getFinancialStatistics(
    ...
    if (period === "year")
      groupExpr = "DATE_FORMAT(FROM_UNIXTIME(add_time), '%Y')";
    ...
    if (period === "week")
      refundGroupExpr = "YEARWEEK(FROM_UNIXTIME(pr.add_time), 1)";
    if (period === "month")
      refundGroupExpr = "DATE_FORMAT(FROM_UNIXTIME(pr.add_time), '%Y-%m')";
    if (period === "year")
      refundGroupExpr = "DATE_FORMAT(FROM_UNIXTIME(pr.add_time), '%Y')";
  ...
  async getInventoryStatistics(shopId: number) {
    const [totalProducts, activeProducts, lowStockProducts, outOfStock] =
      await Promise.all([
        ...
        this.prisma.product.count({
          where: { shop_id: shopId, is_delete: 0, product_status: 1 } as any,
        }),
        this.prisma.product.count({
          where: {
            shop_id: shopId,
        ...
        this.prisma.product.count({
          where: { shop_id: shopId, is_delete: 0, product_stock: 0 } as any,
        }),
      ]);
    return {
      totalProducts,
  ...
  async getMarketingStatistics(
    ...
    const [promoteProducts, couponUsedOrders] = await Promise.all([
      this.prisma.product.count({
        where: { shop_id: shopId, is_delete: 0, is_promote: { gt: 0 } } as any,
      }),
      this.prisma.order.count({
        where: {
          shop_id: shopId,
  ...
  async getComparisonData(
    ...
    const currentStart = new Date(
      base.getTime() - (rangeDays - 1) * 24 * 60 * 60 * 1000,
    );
    currentStart.setHours(0, 0, 0, 0);
    const currentEnd = new Date(base);
    currentEnd.setHours(23, 59, 59, 999);
    const prevStart = new Date(
      currentStart.getTime() - rangeDays * 24 * 60 * 60 * 1000,
    );
    const prevEnd = new Date(
      currentEnd.getTime() - rangeDays * 24 * 60 * 60 * 1000,
    );
    ...
    if (type === "users") {
      const [curr, prev] = await Promise.all([
        this.prisma.user.count({
          where: {
            reg_time: { gte: ts(currentStart), lte: ts(currentEnd) },
          } as any,
        }),
        this.prisma.user.count({
          where: { reg_time: { gte: ts(prevStart), lte: ts(prevEnd) } } as any,
        }),
      ]);
      return {
        current: curr,
      ...
      const [curr, prev] = await Promise.all([
        this.prisma.order.count({
          where: {
            shop_id: shopId,
            is_del: 0,
            add_time: { gte: ts(currentStart), lte: ts(currentEnd) },
          } as any,
        }),
        this.prisma.order.count({
          where: {
            shop_id: shopId,
            is_del: 0,
            add_time: { gte: ts(prevStart), lte: ts(prevEnd) },
          } as any,
        }),
      ]);
      return {
    ...
    return {
      ...
      rate: prevVal ? (currVal - prevVal) / prevVal : 1,
    } as any;
  }
  ...
  async getTrendsAnalysis(
    ...
    const metrics = (query?.metrics || "sales,orders")
      .split(",")
      .map((s) => s.trim());
    const { period = "day" } = query || {};
    const now = new Date();
    ...
    if (metrics.includes("orders"))
      result.orders = base.map((r) => Number(r.orders || 0));
    if (metrics.includes("sales"))
      result.sales = base.map((r) => Number(r.sales || 0));
  ...
  async exportReport(
    ...
    const period =
      query.report_type === "daily"
        ? "day"
        : query.report_type === "weekly"
          ? "week"
          : query.report_type === "monthly"
            ? "month"
            : "year";
    ...
    for (const r of financial as any[]) {
      lines.push(
        `${r.period},${r.paidAmount},${r.totalAmount},${r.refundAmount}`,
      );
    }
    lines.push("");
