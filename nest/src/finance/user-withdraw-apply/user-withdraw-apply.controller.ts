...
export class UserWithdrawApplyController {
  ...
  async statistics(@Query() queryDto: UserWithdrawApplyQueryDto) {
    const statistics = await this.userWithdrawApplyService.getStatistics(queryDto);
    return {
      code: 200,
      msg: '获取成功',
      data: statistics,
  ...
  async getUserHistory(@Request() req, @Query() queryDto: UserWithdrawApplyQueryDto) {
    const userId = req.user.user_id;
    const result = await this.userWithdrawApplyService.getUserWithdrawHistory(userId, queryDto);
    return {
      code: 200,
      msg: '获取成功',
      data: result,
  ...
  @ApiBody({ type: CreateUserWithdrawApplyDto })
  async userApply(@Request() req, @Body() createDto: CreateUserWithdrawApplyDto) {
    const userId = req.user.user_id;
  ...
  async legacyUpdate(@Param('id') id: string, @Body() updateDto: UpdateUserWithdrawApplyDto) {
    const applyId = parseInt(id, 10);
    const apply = await this.userWithdrawApplyService.update(applyId, updateDto);
    return {
      code: 200,
      msg: '更新成功',
      data: apply,
