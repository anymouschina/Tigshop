...
export class UserRechargeOrderController {
  constructor(private readonly userRechargeOrderService: UserRechargeOrderService) {}
  ...
  async statistics(@Query() queryDto: UserRechargeOrderQueryDto) {
    const statistics = await this.userRechargeOrderService.getStatistics(queryDto);
    return {
      code: 200,
      msg: '获取成功',
      data: statistics,
  ...
  async getUserHistory(@Request() req, @Query() queryDto: UserRechargeOrderQueryDto) {
    const userId = req.user.user_id;
    const result = await this.userRechargeOrderService.getUserRechargeHistory(userId, queryDto);
    return {
      code: 200,
      msg: '获取成功',
      data: result,
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        paymentType: { type: 'string', enum: ['alipay', 'wechat', 'balance', 'bank'] },
      },
    },
  })
  async userCreate(@Request() req, @Body() body: { amount: number; postscript?: string; paymentType?: string }) {
    const userId = req.user.user_id;
  ...
  async legacyUpdate(@Param('id') id: string, @Body() updateDto: UpdateUserRechargeOrderDto) {
    const orderId = parseInt(id, 10);
    const order = await this.userRechargeOrderService.update(orderId, updateDto);
    return {
      code: 200,
      msg: '更新成功',
      data: order,
