...
export class UserInvoiceController {
  ...
  async getUserInvoiceHistory(@Request() req, @Query() queryDto: UserInvoiceQueryDto) {
    const userId = req.user.user_id;
    const result = await this.userInvoiceService.getUserInvoiceHistory(userId, queryDto);
    return {
      code: 200,
      msg: '获取成功',
      data: result,
  ...
  async legacyUpdate(@Param('id') id: string, @Body() updateDto: UpdateUserInvoiceDto) {
    const invoiceId = parseInt(id, 10);
