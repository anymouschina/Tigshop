...
export class MessageTypeController {
  ...
  async getMessageTypeList(@Query() query: MessageTypeQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging ? this.messageTypeService.getFilterCount(query) : Promise.resolve(records?.length || 0),
    ]);
  ...
  async updateMessageType(@Body() updateDto: UpdateMessageTypeDto) {
    const result = await this.messageTypeService.update(updateDto.message_id, updateDto);
  ...
  async updateMessageTypeField(@Body() updateDto: UpdateMessageTypeFieldDto) {
    const result = await this.messageTypeService.updateField(updateDto.message_id, updateDto.field, updateDto.value);
  ...
  async batchDeleteMessageType(@Body() batchDto: BatchDeleteMessageTypeDto) {
    const result = await this.messageTypeService.batchDelete(batchDto.message_ids);
  ...
  @ApiResponse({ status: 200, description: '获取成功' })
  async getMessageTypesBySendType(@Param('sendType') sendType: MessageSendType) {
    const result = await this.messageTypeService.getMessageTypesBySendType(sendType);
