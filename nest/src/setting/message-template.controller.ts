...
export class MessageTemplateController {
  ...
  async getMessageTemplateList(@Query() query: MessageTemplateQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging
        ? this.messageTemplateService.getFilterCount(query)
  ...
  async getAllMessageTemplates(
    @Query() query: Partial<MessageTemplateQueryDto>,
  ) {
    const filter = {
      ...query,
      paging: false,
  ...
  async updateMessageTemplate(@Body() updateDto: UpdateMessageTemplateDto) {
    const result = await this.messageTemplateService.update(
      updateDto.id,
      updateDto,
    );
  ...
  @ApiResponse({ status: 200, description: "更新成功" })
  async updateMessageTemplateField(
    @Body() updateDto: UpdateMessageTemplateFieldDto,
  ) {
    const result = await this.messageTemplateService.updateField(
      updateDto.id,
      updateDto.field,
      updateDto.value,
    );
  ...
  @ApiResponse({ status: 200, description: "删除成功" })
  async batchDeleteMessageTemplate(
    @Body() batchDto: BatchDeleteMessageTemplateDto,
  ) {
  ...
  async getTemplatesByMessageType(@Param("messageId") messageId: string) {
    const result =
      await this.messageTemplateService.getTemplatesByMessageType(messageId);
