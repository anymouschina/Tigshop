...
export class ConfigController {
  ...
  async getConfigList(@Query() query: ConfigQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging ? this.configService.getFilterCount(query) : Promise.resolve(records?.length || 0),
    ]);
  ...
  async updateConfigField(@Body() updateDto: UpdateConfigFieldDto) {
    const result = await this.configService.updateField(updateDto.id, updateDto.field, updateDto.value);
  ...
  @ApiQuery({ name: 'biz_codes', required: true, description: '业务代码数组，用逗号分隔' })
  @ApiResponse({ status: 200, description: '获取成功' })
  ...
  async getNumberConfig(
    ...
    const result = await this.configService.getNumberConfig(bizCode, defaultNum);
  ...
  async getBooleanConfig(
    ...
    const result = await this.configService.getBooleanConfig(bizCode, defaultBool);
