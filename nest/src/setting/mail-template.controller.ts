...
export class MailTemplateController {
  ...
  async getMailTemplateList(@Query() query: MailTemplateQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging
        ? this.mailTemplateService.getFilterCount(query)
  ...
  async updateMailTemplate(@Body() updateDto: UpdateMailTemplateDto) {
    const result = await this.mailTemplateService.update(
      updateDto.template_id,
      updateDto,
    );
  ...
  async updateMailTemplateField(@Body() updateDto: UpdateMailTemplateFieldDto) {
    const result = await this.mailTemplateService.updateField(
      updateDto.template_id,
      updateDto.field,
      updateDto.value,
    );
  ...
  async batchDeleteMailTemplate(@Body() batchDto: BatchDeleteMailTemplateDto) {
    const result = await this.mailTemplateService.batchDelete(
      batchDto.template_ids,
    );
  ...
  async getTemplateByCode(@Param("templateCode") templateCode: string) {
    const result =
      await this.mailTemplateService.getTemplateByCode(templateCode);
  ...
  async previewTemplate(
    ...
    const result = await this.mailTemplateService.previewTemplate(
      templateId,
      parsedVariables,
    );
