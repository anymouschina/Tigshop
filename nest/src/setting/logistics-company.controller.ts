...
import { LogisticsCompanyService, LOGISTICS_SHOW_STATUS } from './logistics-company.service';
import {
...
export class LogisticsCompanyController {
  constructor(private readonly logisticsCompanyService: LogisticsCompanyService) {}
  ...
  @ApiQuery({ name: 'logistics_id', required: false, description: '物流公司ID' })
  ...
  async getLogisticsCompanyList(@Query() query: LogisticsCompanyQueryDto) {
    const [records, total] = await Promise.all([
      ...
      query.paging ? this.logisticsCompanyService.getFilterCount(query) : Promise.resolve(records?.length || 0),
    ]);
  ...
  @ApiQuery({ name: 'logistics_id', required: false, description: '物流公司ID' })
  @ApiResponse({ status: 200, description: '获取成功' })
  async getAllLogisticsCompanies(@Query() query: Partial<LogisticsCompanyQueryDto>) {
    const filter = {
      ...query,
      paging: false,
    };
  ...
  async updateLogisticsCompany(@Body() updateDto: UpdateLogisticsCompanyDto) {
    const result = await this.logisticsCompanyService.update(updateDto.logistics_id, updateDto);
  ...
  @ApiResponse({ status: 200, description: '更新成功' })
  async updateLogisticsCompanyField(@Body() updateDto: UpdateLogisticsCompanyFieldDto) {
    const result = await this.logisticsCompanyService.updateField(updateDto.id, updateDto.field, updateDto.value);
  ...
  @ApiResponse({ status: 200, description: '删除成功' })
  async batchDeleteLogisticsCompany(@Body() batchDto: BatchDeleteLogisticsCompanyDto) {
  ...
  async getAvailableCompanies() {
    const companies = await this.logisticsCompanyService.getAllAvailableCompanies();
