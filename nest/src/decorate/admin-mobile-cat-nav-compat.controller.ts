...
export class AdminMobileCatNavCompatController {
  ...
  async list(@Query() query: any) {
    ...
    const sortField = this.toSnakeSortField(query.sortField || query.sort_field);
    ...
    // 关联分类名称
    const catIds = Array.from(new Set(recordsRaw.map((r) => r.category_id).filter(Boolean)));
    ...
    if (catIds.length) {
      ...
      catNameMap = Object.fromEntries(cats.map((c) => [c.category_id, c.category_name]));
    }
  ...
  async detail(@Query("id") id: number) {
    ...
    let img_url = this.parseMaybeJson(record.img_url);
    let child_cat_ids = this.parseMaybeJson(record.child_cat_ids);
    let brand_ids = this.parseMaybeJson(record.brand_ids);
  ...
  async create(@Body() body: any) {
    const data: any = {
      ...
      child_cat_ids: JSON.stringify(body.child_cat_ids ?? body.childCatIds ?? []),
  ...
  async update(@Body() body: any) {
    ...
    if (body.category_id != null || body.categoryId != null)
      data.category_id = this.coerceNumber(body.category_id ?? body.categoryId, 0);
    if (body.cat_color != null || body.catColor != null) data.cat_color = body.cat_color ?? body.catColor;
    ...
    if (body.child_cat_ids != null || body.childCatIds != null)
      data.child_cat_ids = JSON.stringify(body.child_cat_ids ?? body.childCatIds ?? []);
    ...
    if (body.sort_order != null || body.sortOrder != null)
      data.sort_order = this.coerceNumber(body.sort_order ?? body.sortOrder, 50);
  ...
  async updateField(@Body() body: any) {
    ...
    const val = body.val;
    const data: any = { [field]: field === "sort_order" ? this.coerceNumber(val, 50) : this.coerceNumber(val, 0) };
    await this.prisma.mobile_cat_nav.update({ where: { mobile_cat_nav_id: id }, data });
  ...
  async del(@Body("id") id: number) {
    ...
    await this.prisma.mobile_cat_nav.delete({ where: { mobile_cat_nav_id: navId } });
    return { code: 0, message: "success", data: true };
  }
  ...
  async batch(@Body() body: any) {
    ...
    if (type === "del" || type === "delete") {
      await this.prisma.mobile_cat_nav.deleteMany({ where: { mobile_cat_nav_id: { in: ids } } });
