...
export class AdminPcCatFloorCompatController {
  ...
  async list(@Query() q: any) {
    ...
    const [records, total] = await Promise.all([
      this.prisma.pc_cat_floor.findMany({
        where,
  ...
  async detail(@Query("id") id: number) {
    const item = await this.prisma.pc_cat_floor.findUnique({
      where: { cat_floor_id: this.num(id, 0) },
    });
  ...
  async update(@Body() body: any) {
    ...
    if (body.category_ids != null)
      data.category_ids = String(body.category_ids);
    if (body.category_names != null)
      data.category_names = String(body.category_names);
    ...
    if (body.sort_order != null)
      data.sort_order = this.num(body.sort_order, 50);
    ...
    const updated = await this.prisma.pc_cat_floor.update({
      where: { cat_floor_id: id },
      data,
    });
  ...
  async updateField(@Body() body: any) {
    ...
    await this.prisma.pc_cat_floor.update({
      where: { cat_floor_id: id },
      data,
    });
  ...
  async del(@Body("id") id: number) {
    await this.prisma.pc_cat_floor.delete({
      where: { cat_floor_id: this.num(id, 0) },
    });
    return { code: 0, message: "success", data: true };
  }
  ...
  async batch(@Body() body: any) {
    ...
    if (type === "del" || type === "delete") {
      await this.prisma.pc_cat_floor.deleteMany({
        where: { cat_floor_id: { in: ids } },
      });
      return { code: 0, message: "success", data: true };
    }
