...
function detectUserFrom(req: Request & any): FromStr {
  ...
  if (
    ["wechat", "mp", "mp-wechat", "official", "official-account"].includes(hint)
  )
    return "wechat";
  if (["mini", "miniapp", "miniprogram", "wxmini", "weapp"].includes(hint))
    return "miniapp";
  ...
  const isMiniFlag =
    String(headers["x-wechat-miniprogram"] || "").toLowerCase() === "true" ||
    ua.includes("miniprogram") ||
    ua.includes("mini program");
  if (isWeChat && isMiniFlag) return "miniapp";
  if (isWeChat) return "wechat";

  // 3) Native app hints (custom headers preferred)
  const hasAppHeader = Boolean(
    headers["x-app-client"] || headers["x-app-version"],
  );
  ...
  const isMobile =
    /android|iphone|ipad|ipod|mobile|mqqbrowser|ucbrowser|miuibrowser/.test(ua);
  ...
  use(req: Request & any, _res: Response, next: NextFunction) {
    try {
      ...
      if (
        req &&
        req.query &&
        req.query.userFrom == null &&
        req.query.user_from == null
      ) {
      ...
      if (
        req &&
        req.body &&
        req.body.userFrom == null &&
        req.body.user_from == null
      ) {
