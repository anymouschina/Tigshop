...
import { CreateTipsManageDto, UpdateTipsManageDto, QueryTipsManageDto } from './dto/tips-manage.dto';
...
export class TipsManageController {
  ...
  @Put('update/:id')
  async update(@Param('id') id: number, @Body() updateData: UpdateTipsManageDto) {
    const result = await this.tipsmanageService.updateTipsManage(id, updateData);
    if (result) {
