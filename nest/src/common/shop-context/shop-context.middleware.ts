...
export class ShopContextMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    const headerVal = (req.headers['x-shop-id'] ?? req.headers['x-shopid']) as any;
    ...
    if (adminId) {
      try {
        const row = await this.prisma.admin_user.findUnique({ where: { admin_id: adminId }, select: { admin_type: true, shop_id: true } });
        if (row) {
    ...
    shopContext.run({ shopId: headerShopId, isSuperAdmin: isSuper }, () => next());
  }
}
