...
export class CsrfController {
  ...
  async validateToken(@Body() validateTokenDto: { token: string, user_id?: number }) {
    return await this.csrfService.validateToken(validateTokenDto.token, validateTokenDto.user_id);
  }

  @Post('refresh-token')
  @ApiOperation({ summary: '刷新CSRF令牌' })
  async refreshToken(@Body() refreshTokenDto: { old_token: string, user_id?: number }) {
    return await this.csrfService.refreshToken(refreshTokenDto.old_token, refreshTokenDto.user_id);
  }
