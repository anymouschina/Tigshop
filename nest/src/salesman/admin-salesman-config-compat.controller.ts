...
export class AdminSalesmanConfigCompatController {
  ...
  @ApiQuery({
    name: "code",
    required: true,
    description: "配置编码：salesmanConfig | salesmanSettlement",
  })
  ...
  async detail(@Req() req: any, @Query("code") code: string) {
    ...
    const row = await this.prisma.salesman_config.findFirst({
      where: { shop_id: shopId, code },
    });
    const data = row?.data
      ? this.parseMaybeJson(row.data)
      : this.dftConfig(code);
  ...
  async save(@Req() req: any, @Query("code") code: string, @Body() body: any) {
    const shopId = await this.panel.getUserShopId(req.user?.userId);
    const exists = await this.prisma.salesman_config.findFirst({
      where: { shop_id: shopId, code },
    });
    const payload = {
      shop_id: shopId,
    ...
    if (exists) {
      await this.prisma.salesman_config.update({
        where: { id: exists.id },
        data: payload,
      });
    } else {
