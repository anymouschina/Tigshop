...
export class AdminSalesmanCompatController {
  ...
  const groupIds = Array.from(new Set(rows.map((r) => r.group_id).filter((x) => this.coerceNumber(x, 0) > 0)));
    ...
    const [users, pidUsers, groups, customerCounts, inviteCounts, commissionAgg] = await Promise.all([
      ...
      pids.length
        ? this.prisma.user.findMany({
            ...
            select: { user_id: true, mobile: true, username: true, nickname: true, avatar: true, distribution_register_time: true },
          })
        : Promise.resolve([]),
      groupIds.length
        ? this.prisma.salesman_group.findMany({ where: { group_id: { in: groupIds } }, select: { group_id: true, group_name: true } })
        : Promise.resolve([]),
      salesmanIds.length
        ? this.prisma.salesman_customer.groupBy({ by: ["salesman_id"], _count: { salesman_customer_id: true }, where: { salesman_id: { in: salesmanIds } } })
        : Promise.resolve([]),
      userIds.length
    ...
    const customerCountMap = new Map(customerCounts.map((c: any) => [c.salesman_id, c._count?.salesman_customer_id || 0]));
    const inviteCountMap = new Map(inviteCounts.map((c: any) => [c.pid, c._count?.salesman_id || 0]));
    ...
    const records = rows.map((r) => {
      ...
      return {
        ...
        baseUserInfo: base
          ...
          : null,
        groupInfo: grp ? { groupId: grp.group_id, groupName: grp.group_name || "" } : null,
        pidUserInfo: pidUser
          ? {
              ...
              distribution_register_time: pidUser.distribution_register_time ?? 0,
            }
          : null,
        customer: [],
      };
  ...
  async create(@Req() req: any, @Body() body: any) {
    ...
    // 可选：校验用户是否存在
    const user = await this.prisma.user.findUnique({ where: { user_id: userId }, select: { user_id: true } });
    if (!user) return { code: 404, message: "user not found", data: null };

    // 幂等：同店铺下同用户若已是分销员，则直接返回成功
    const existed = await this.prisma.salesman.findFirst({ where: { shop_id: shopId, user_id: userId } });
    ...
    const data: any = {
      ...
      group_id: body.groupId !== undefined ? this.coerceNumber(body.groupId, 0) : 0,
  ...
  async ranking(@Req() req: any, @Query() query: any) {
    ...
    const [rows, total] = await Promise.all([
      this.prisma.salesman.findMany({ where: { shop_id: shopId }, orderBy: { salesman_id: "desc" } }),
    ...
    const [users, customerAgg, orderAgg] = await Promise.all([
      userIds.length
        ? this.prisma.user.findMany({ where: { user_id: { in: userIds } }, select: { user_id: true, username: true, nickname: true } })
        : Promise.resolve([]),
      salesmanIds.length
        ? this.prisma.salesman_customer.groupBy({ by: ["salesman_id"], _count: { salesman_customer_id: true }, where: { salesman_id: { in: salesmanIds } } })
        : Promise.resolve([]),
      salesmanIds.length
        ? this.prisma.salesman_order.groupBy({ by: ["salesman_id"], _count: { salesman_order_id: true }, _sum: { order_amount: true }, where: { salesman_id: { in: salesmanIds } } })
        : Promise.resolve([]),
    ]);

    const userMap = new Map(users.map((u) => [u.user_id, u]));
    const customerMap = new Map(customerAgg.map((c: any) => [c.salesman_id, c._count?.salesman_customer_id || 0]));
    const orderCountMap = new Map(orderAgg.map((o: any) => [o.salesman_id, o._count?.salesman_order_id || 0]));
    const orderAmountMap = new Map(orderAgg.map((o: any) => [o.salesman_id, o._sum?.order_amount || 0]));
    ...
    const records = rows.map((r) => {
      ...
      return {
        username: u?.username || "",
        nickname: u?.nickname || "",
        totalSaleAmount: Number.isFinite(Number(totalSaleAmount)) ? Number(totalSaleAmount) : 0,
        totalCustomers,
