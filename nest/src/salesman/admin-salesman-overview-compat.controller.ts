...
export class AdminSalesmanOverviewCompatController {
  ...
  async coreSummary(@Req() req: any, @Query() query: any) {
    ...
    const newSalesmanCount = await this.prisma.salesman.count({
      where: { shop_id: shopId, add_time: { gte: from } },
    });
    // 分销总销售额（累计）
    const orders = await this.prisma.salesman_order.findMany({
      where: { salesman_id: { gt: 0 } },
    });
    const salesmanAmount = orders.reduce(
      (sum, o) => sum + Number(o.order_amount || 0),
      0,
    );
    // 分销佣金支出（累计已结算）
    const settled = orders.filter((o) => Number(o.status) === 1);
    const salesmanCommission = settled.reduce(
      (sum, o) => sum + Number(o.amount || 0),
      0,
    );
    // 客户数（近30天新增客户去重）
    const customNum = await this.prisma.salesman_customer.groupBy({
      by: ["user_id"],
      where: { add_time: { gte: from } },
    });
    return {
      code: 0,
      message: "success",
  ...
  async coreTrend(@Req() req: any, @Query() query: any) {
    ...
    const now = new Date();
    let days = 1;
    if (dateType === 1) days = 1;
    else if (dateType === 2) days = 7;
    else if (dateType === 3) days = 30;
    else if (dateType === 4) days = 180;
    else days = 1;
    ...
    return {
      code: 0,
      message: "success",
      data: { horizontalAxis, longitudinalAxis },
    };
  }
}
