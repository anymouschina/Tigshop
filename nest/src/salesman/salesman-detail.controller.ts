...
export class SalesmanDetailController {
  ...
  @ApiQuery({ name: 'salesmanId', required: false, description: '分销员ID，不传则取当前用户对应的分销员' })
  async detail(@Req() req: any, @Query('salesmanId') salesmanId?: string) {
    ...
    if (!sid || !Number.isFinite(sid)) {
      ...
      const selfSalesman = await this.prisma.salesman.findFirst({ where: { user_id: userId } });
    ...
    // 并行聚合
    const [totalCommissionRow, totalCustomerRow, totalInvite, userBase, groupInfo] = await Promise.all([
      ...
      salesman.user_id ? this.prisma.user.findUnique({ where: { user_id: salesman.user_id }, select: { user_id: true, username: true, nickname: true, avatar: true, mobile: true } }) : null,
    ...
    const data = {
      ...
      group: groupInfo ? { groupId: groupInfo.group_id, groupName: groupInfo.group_name } : null,
    };
