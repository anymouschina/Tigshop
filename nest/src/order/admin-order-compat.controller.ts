...
export class AdminOrderCompatController {
  ...
  async logCreate(@Body() body: any, @Req() req: any) {
    ...
    if ((!id || !Number.isFinite(id)) && body.orderSn) {
      try {
        const found = await this.svc.prisma.order.findUnique({
          where: { order_sn: String(body.orderSn) },
        });
  ...
  @Authorities("order")
  @ApiOperation({
    summary: "保存订单导出字段设置（admin 兼容 - PHP 路径别名）",
  })
  ...
  async aftersalesUpdate(@Body() body: any, @Req() req: any) {
    ...
    const ok = await this.aftersalesSvc.agreeOrRefuse(
      Number(body.aftersale_id ?? body.id),
      {
  ...
  async aftersalesComplete(@Body() body: any, @Req() req: any) {
    const ok = await this.aftersalesSvc.complete(
      Number(body.id ?? body.aftersale_id),
      req?.user?.userId ?? 0,
    );
    return ok ? { code: 0, message: "success" } : { code: 1, message: "error" };
  }
  ...
  async parentDetailAlias(@Query("id") id: string) {
    return this.parentDetail(id);
  }
  ...
  async deliverAlias(@Body() body: any, @Req() req: any) {
    return this.deliver(body, req);
  }
  ...
  async confirmReceipt(@Body() body: any, @Req() req: any) {
    const id = Number(body.id ?? body.orderId);
    await this.svc.confirmReceipt(
      id,
      body.shippingStatus ?? body.shipping_status,
      req?.user?.username,
      req?.user?.userId ?? req?.user?.adminId,
    );
    return { code: 0, message: "success" };
  ...
  async confirmReceiptAlias(@Body() body: any, @Req() req: any) {
    return this.confirmReceipt(body, req);
  }
  ...
  async modifyConsignee(@Body() body: any, @Req() req: any) {
    const id = Number(body.id ?? body.orderId);
    await this.svc.modifyConsignee(
      id,
      body,
      req?.user?.username,
      req?.user?.userId ?? req?.user?.adminId,
  ...
  async modifyConsigneeAlias(@Body() body: any, @Req() req: any) {
    return this.modifyConsignee(body, req);
  }
  ...
  async modifyShippingAlias(@Body() body: any, @Req() req: any) {
    return this.modifyShipping(body, req);
  }
  ...
  async modifyMoneyAlias(@Body() body: any, @Req() req: any) {
    return this.modifyMoney(body, req);
  }
  ...
  async cancelOrder(@Body() body: any, @Req() req: any) {
    const id = Number(body.id ?? body.orderId);
    await this.svc.cancelOrder(
      id,
      body.reason ?? body.remark,
      body.orderStatus ?? body.order_status,
      req?.user?.username,
      req?.user?.userId ?? req?.user?.adminId,
    );
    return { code: 0, message: "success" };
  ...
  async cancelOrderAlias(@Body() body: any, @Req() req: any) {
    return this.cancelOrder(body, req);
  }
  ...
  async setConfirmAlias(@Body() body: any, @Req() req: any) {
    return this.setConfirm(body, req);
  }
  ...
  async delOrderAlias(@Body() body: any, @Req() req: any) {
    return this.delOrder(body, req);
  }
  ...
  async splitStoreOrderAlias(@Body() body: any) {
    return this.splitStoreOrder(body);
  }
  ...
  async setPaid(@Body() body: any, @Req() req: any) {
    const id = Number(body.id ?? body.orderId);
    await this.svc.setPaid(
      id,
      body.payStatus ?? body.pay_status,
      req?.user?.username,
      req?.user?.userId ?? req?.user?.adminId,
    );
    return { code: 0, message: "success" };
  ...
  async setPaidAlias(@Body() body: any, @Req() req: any) {
    return this.setPaid(body, req);
  }
  ...
  async modifyProductAlias(@Body() body: any, @Req() req: any) {
    return this.modifyProduct(body, req);
  }
  ...
  async getAddProductInfoAlias(@Body() body: any) {
    return this.getAddProductInfo(body);
  }
  ...
  async setAdminNote(@Body() body: any, @Req() req: any) {
    const id = Number(body.id ?? body.orderId);
    await this.svc.setAdminNote(
      id,
      body.note ?? body.adminNote ?? body.admin_note,
      req?.user?.username,
      req?.user?.userId ?? req?.user?.adminId,
    );
    return { code: 0, message: "success" };
  ...
  async setAdminNoteAlias(@Body() body: any, @Req() req: any) {
    return this.setAdminNote(body, req);
  }
  ...
  async orderPrintBillAlias() {
    return this.orderPrintBill();
  }
  ...
  async getExportItemListAlias() {
    return this.getExportItemList();
  }
  ...
  async exportItemInfoAlias(@Req() req: any) {
    return this.exportItemInfo(req);
  }
  ...
  async orderExport(
    ...
    if (raw) {
      if (Array.isArray(raw)) fields = raw.map(String);
        ...
        try {
          ...
          if (Array.isArray(arr)) fields = arr.map(String);
          else
            fields = String(raw)
              .split(",")
              .map((s) => s.trim())
    ...
    if (!fields.length)
      fields = await this.svc.getExportItemInfo(req?.user?.userId ?? 0);
    const { headers, rows } = await this.svc.buildOrderExportRows(
      query,
      fields,
    );
    // 生成 CSV（BOM + CRLF）
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="order-export-${new Date().toISOString().slice(0, 10)}.csv"`,
    );
  ...
  async batch(@Body() body: any, @Req() req: any) {
    ...
    if (Array.isArray(raw))
      ids = raw.map((x) => Number(x)).filter((n) => Number.isFinite(n));
    else if (typeof raw === "string")
      ids = raw
        .split(",")
        .map((s) => Number(s.trim()))
    ...
    const result = await this.svc.batchOperation(
      String(type),
      ids,
      data,
      req?.user?.username,
  ...
  async batchAlias(@Body() body: any, @Req() req: any) {
    return this.batch(body, req);
  }
  ...
  async severalDetailAlias(@Query("ids") idsParam: any) {
    return this.severalDetail(idsParam);
  }
  ...
  async printSeveral(@Query("ids") idsParam: any) {
    ...
    const data = await Promise.all(
      ids.map((id) => this.svc.getOrderPrintData(id)),
    );
    return { code: 0, message: "success", data };
  }
  ...
  async printSeveralAlias(@Query("ids") idsParam: any) {
    return this.printSeveral(idsParam);
  }
  ...
  async shippingInfoAlias(@Query("id") id: string) {
    return this.shippingInfo(id);
  }
  ...
  async getOrderPageConfigAlias() {
    return this.getOrderPageConfig();
  }
}
