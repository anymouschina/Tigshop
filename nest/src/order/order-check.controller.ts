...
export class OrderCheckController {
  ...
  async index(@Request() req, @Body() body: {
    ...
    let useCouponIds: number[] = [];
    let selectUserCouponIds: number[] = [];
        ...
        if (shopCart.used_promotions) {
          for (const usedPromotion of shopCart.used_promotions) {
            if (usedPromotion.type === 2) {
              ...
              const userCouponId = await this.orderCheckService.getUserCouponIdByCouponId(
                userId,
    ...
    const result = {
      ...
      available_payment_type: await this.orderCheckService.getAvailablePaymentType(),
  ...
  async update(@Request() req, @Body() body: {
    ...
    const cartList = await this.orderCheckService.getStoreCarts('', params.flow_type);
    if (!cartList.carts || cartList.carts.length === 0) {
      throw new HttpException('您还未选择商品！', HttpStatus.BAD_REQUEST);
    }
  ...
  async getStoreShippingType(@Query() query: {
    address_id?: number;
    shipping_type?: any;
    pay_type_id?: number;
    use_point?: number;
  ...
  async updateCoupon(@Request() req, @Body() body: {
    address_id?: number;
    shipping_type?: any;
    pay_type_id?: number;
    use_point?: number;
    ...
    if (body.use_default_coupon_ids === 1 && params.use_coupon_ids.length === 0) {
      useDefaultCoupon = 1;
    }
  ...
  async submit(@Request() req, @Body() body: {
    ...
    if (closeOrder === 1) {
      throw new HttpException('商城正在维护已停止下单！', HttpStatus.SERVICE_UNAVAILABLE);
    }
  ...
  async getInvoice(@Request() req, @Query() query: {
    invoice_type?: number;
    title_type?: number;
  }) {
    const userId = req.user.userId;
