...
import {
  ServiceState,
  AllowedTransitions,
  ServiceStateToOrderStatus,
  LogEvent,
  extractServiceState,
} from "./dine-order.constants";
...
export class DineOrderService {
  constructor(
    ...
    private readonly pickup: PickupNoService,
    private readonly redis: RedisService,
    private readonly events: DineOrderEventsService,
  ) {}
  ...
  private async getIdemResult(key: string) {
    try {
      return await this.redis["redis"].get(key);
    } catch {
      return null;
    }
  }
  async create(
    userId: number,
    shopId: number,
    dto: CreateDineOrderDto,
    idemToken?: string,
  ) {
    if (![2, 3].includes(dto.orderType))
      throw new BadRequestException("orderType must be 2(dine) or 3(takeout)");
    if (dto.orderType === 2 && !dto.tableNo)
      throw new BadRequestException("堂食需要提供桌号");
    if (dto.orderType === 3 && dto.tableNo)
      throw new BadRequestException("外带不应提供桌号");
    ...
    for (const item of itemsInput) {
      ...
      if (Number(prod.is_delete) === 1)
        throw new BadRequestException(`商品已下架: ${item.productId}`);
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of itemsInput) {
        ...
        if (item.skuId) {
          const affected =
            await tx.$executeRaw`UPDATE product_sku SET sku_stock = sku_stock - ${Number(item.quantity)} WHERE sku_id=${Number(item.skuId)} AND sku_stock >= ${Number(item.quantity)}`;
          if (affected !== 1) throw new BadRequestException("规格库存不足");
        }
        const affectedProd =
          await tx.$executeRaw`UPDATE product SET product_stock = product_stock - ${Number(item.quantity)} WHERE product_id=${prod.product_id} AND product_stock >= ${Number(item.quantity)}`;
        if (affectedProd !== 1) throw new BadRequestException("商品库存不足");
      }
      await tx.order_log.create({
        data: {
          order_id: order.order_id,
          order_sn: order.order_sn,
          user_id: userId,
    ...
    try {
      ...
      this.events.emit({
        ...
        rootOrderId: created.order_id,
        shopId,
        userId,
        serviceState: extState,
        dineScene: extension.dineScene,
        tableNo: created.table_no,
        pickupNo: created.pickup_no,
  ...
  async append(
    ...
    const idemKey = this.buildIdemKey(
      "append",
      userId,
      shopId,
      idemToken ? `${parentOrderId}:${idemToken}` : undefined,
    );
    if (idemKey) {
      const first = await this.enterIdempotency(idemKey);
      if (!first) {
        ...
        const recent = await this.prisma.order.findFirst({
          where: { parent_order_id: parent.order_id },
          orderBy: { order_id: "desc" },
        });
        if (recent) return recent;
      }
    }
    if (![2, 3].includes(Number(parent.order_type)))
      throw new BadRequestException("非堂食/外带订单不支持加单");
    const ext = this.safeJson(parent.order_extension) || {};
    if (
      ext.serviceState &&
      ["COMPLETED", "CANCELED"].includes(ext.serviceState)
    )
      throw new BadRequestException("订单已结束不可加单");
    ...
    const child = await this.prisma.$transaction(async (tx) => {
      ...
      for (const it of items) {
        ...
        if (it.skuId) {
          const affectedSku =
            await tx.$executeRaw`UPDATE product_sku SET sku_stock = sku_stock - ${Number(it.quantity)} WHERE sku_id=${Number(it.skuId)} AND sku_stock >= ${Number(it.quantity)}`;
          if (affectedSku !== 1) throw new BadRequestException("规格库存不足");
        }
        const affectedProd =
          await tx.$executeRaw`UPDATE product SET product_stock = product_stock - ${Number(it.quantity)} WHERE product_id=${p.product_id} AND product_stock >= ${Number(it.quantity)}`;
        if (affectedProd !== 1) throw new BadRequestException("商品库存不足");
      }
      await tx.order_log.create({
        data: {
    ...
    try {
      ...
      this.events.emit({
        ...
        rootOrderId: parent.parent_order_id
          ? parent.parent_order_id
          : parent.order_id,
        parentOrderId: parent.order_id,
        shopId,
        userId: user,
        dineScene: ext.dineScene,
        tableNo: child.table_no,
        pickupNo: child.pickup_no,
  ...
  async changeTable(orderId: number, newTableNo: string) {
    ...
    if (Number(order.order_type) !== 2)
      throw new BadRequestException("只有堂食订单可换桌");
    const updated = await this.prisma.order.update({
      where: { order_id: orderId },
      data: { table_no: newTableNo },
    });
    await this.log(
      order.order_id,
      order.order_sn,
      order.user_id,
      order.shop_id,
      { event: LogEvent.CHANGE_TABLE, from: order.table_no, to: newTableNo },
    );
    try {
      this.events.emit({
        kind: "CHANGE_TABLE",
  ...
  async updateServiceState(orderId: number, to: string) {
    ...
    if (!allowed[from].includes(to))
      throw new BadRequestException(`状态不允许从 ${from} -> ${to}`);
    ext.serviceState = to;
    ...
    await this.log(
      ...
      order.user_id,
      order.shop_id,
      { event: LogEvent.STATE_CHANGE, from, to },
    );
    try {
      this.events.emit({
        kind: "STATE_CHANGE",
    ...
    return {
      records: active.map((o) => {
        ...
        return {
          ...
          serviceState: extractServiceState(ext),
        };
      }),
    };
  }
  ...
  async pay(orderId: number, userId: number) {
    ...
    await this.log(
      ...
      order.user_id,
      order.shop_id,
      { event: LogEvent.PAY, amount: order.total_amount },
    );
    try {
      this.events.emit({
        kind: "PAY",
  ...
  async cancel(orderId: number, userId: number) {
    ...
    if (Number(order.pay_status) === 1)
      throw new BadRequestException("已支付订单需申请退款");
    if (Number(order.order_status) === 4) return { ok: true, canceled: true };
    const ext = this.safeJson(order.order_extension) || {};
    if (
      [ServiceState.COMPLETED, ServiceState.CANCELED].includes(ext.serviceState)
    )
      return { ok: true };
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const it of items) {
        if (it.sku_id) {
          await tx.product_sku.update({
            where: { sku_id: Number(it.sku_id) },
            data: { sku_stock: { increment: Number(it.quantity) } as any },
          });
        }
        await tx.product.updateMany({
          where: { product_id: Number(it.product_id) },
          data: { product_stock: { increment: Number(it.quantity) } as any },
        });
      }
      ...
      await tx.order.update({
        where: { order_id: orderId },
        data: {
          order_status: ServiceStateToOrderStatus[ServiceState.CANCELED],
          order_extension: JSON.stringify(ext),
        },
      });
      await tx.order_log.create({
        data: {
          ...
          admin_id: 0,
          description: JSON.stringify({ event: LogEvent.CANCEL }),
    ...
    try {
      this.events.emit({
        ...
        shopId: order.shop_id,
        userId: order.user_id,
        serviceState: ServiceState.CANCELED,
        tableNo: order.table_no,
        pickupNo: order.pickup_no,
        orderType: order.order_type,
      });
  ...
  async detail(orderId: number, userId: number) {
    ...
    // 找主单
    const root = base.parent_order_id
      ? await this.prisma.order.findUnique({
          where: { order_id: base.parent_order_id },
        })
      : base;
    const children = await this.prisma.order.findMany({
      where: {
        OR: [{ order_id: root.order_id }, { parent_order_id: root.order_id }],
      },
    });
    ...
    return {
      ...
      orders: children.map((o) => ({
        id: o.order_id,
        isAppend: this.safeJson(o.order_extension).isAppend || false,
  ...
  async rootSummary(orderId: number, userId: number) {
    ...
    const items = await this.prisma.order_item.findMany({
      where: { order_id: { in: ids as any } },
    });
    const totalProduct = orders.reduce(
      (sum, o) => sum + Number(o.product_amount),
      0,
    );
    ...
    return {
      ...
      orders: orders.map((o) => ({
        id: o.order_id,
        isAppend: o.parent_order_id ? true : false,
