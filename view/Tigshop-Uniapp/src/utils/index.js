 ...
 */
export function guid(len = 32, firstU = true, radix) {
    ...
    if (len) {
        ...
        for (let i = 0; i < len; i++)
            uuid[i] = chars[0 | (Math.random() * usedRadix)];
    }
    else {
        let r;
 ...
 * 手机号隐藏
 */
export function mobileConceal(mobile) {
 ...
 * @returns  {AnyObject}
 */
export const formatArguments = (data) => {
    const result = {};
...
export function getElementRect(selector, instance) {
...
export function getDownloadFileInfo(url) {
    return new Promise((resolve, reject) => {
        uni.downloadFile({
            url,
            success: (res) => {
 ...
 * @returns
 */
export function staticResource(path, type = "images") {
    if (isUrl(path)) {
        return path;
    }
...
export const saveImageToMp = (filePath) => {
    return new Promise((resolve, reject) => {
        uni.getSetting({
            success(res) {
...
export const saveImageToApp = (filePath) => {
    return new Promise((resolve, reject) => {
        uni.saveImageToPhotosAlbum({
            filePath,
            success: (res) => {
