...
export const useAnimation = (selector, animation) => {
    const instance = getCurrentInstance();
    const show = ref(false);
    let observer;
    onMounted(() => {
        observer = uni.createIntersectionObserver(instance);
        observer.relativeToViewport({ top: 0, bottom: 10 }).observe(selector, (res) => {
    ...
    const transitionDuration = (index) => {
...
export const useImgInfo = (imgUrl, selector, immediate = true) => {
    const imgratio = ref(0);
    const getImgratio = async (url) => {
        try {
...
//# sourceMappingURL=useDecorate.js.map