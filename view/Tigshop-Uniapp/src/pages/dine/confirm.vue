...
onLoad((q:any)=>{ shopId.value=Number(q.shopId); tableNo.value=q.table||''; const t = Number(q.type); orderType.value = (t===3?3:2); parseItems(q.items); });
...
async function submit(){
  ...
  try{
    ...
    const res = await dineCreate({ orderType: orderType.value, tableNo: tableNo.value, remark: remark.value, items: cart.value.map(c=>({ productId:c.id, quantity:c.quantity })) }, shopId.value, idem);
